...
# === 工具栏（小地图上方）=== 【对准新背景图按钮位置】
BTN_REFRESH = (35, 95, 50, 30)  # 刷新按钮
BTN_MANUAL = (95, 95, 50, 30)  # 手动截取小地图按钮
BTN_PLAN_TOOLBAR = (156, 103, 57, 23)  # 当前使用（方案名/随机）→ 对准背景图"当前使用"按钮，文字才居中
# 【模块B】自动校准按钮（X倍率，同屏三点校准：基点+右800+上500）
BTN_CALIB_AUTO = (235, 95, 40, 30)  # X倍率按钮
...
YELLOW_H_LOW = 20  # 黄色H下限，收紧范围排除偏橙/偏绿的噪声
YELLOW_H_HIGH = 35  # 黄色H上限，从40收紧到35，排除偏绿黄
...
ROUTE_FIELDS = [
    ...
    # 倍率差输入框（弹窗形式，点击右上角图标打开弹窗，这里保留字段定义用于输入处理）
    (150, 300, 120, 35, "num", "scale_x_offset"),  # X偏差（弹窗中）
    (150, 360, 120, 35, "num", "scale_y_offset"),  # Y偏差（弹窗中）
]
...
class MinimapRouteRecorder:
    def __init__(self):
        ...
        self._show_scale_dialog = False  # 是否显示倍率差调整弹窗
        ...
        # 倍率差弹窗按钮区域（右上角）
        self._btn_scale_dialog = (370, 96, 45, 30)  # 倍率差按钮位置（对准新背景图上的倍率差按钮，Y倍率先空着）
        # 平台选择按钮区域（小地图左上方）
        ...
        if os.path.exists(calib_file):
            try:
                ...
                if saved_sx > 0 and saved_sy > 0:
                    ...
                    # 把总倍率加载到基础倍率输入框中（偏差不保存，每次重新输入）
                    self._field_values["scale_x_base"] = "%.4f" % saved_sx
                    self._field_values["scale_y_base"] = "%.4f" % saved_sy
        ...
        self._calib_match_threshold = 0.80   # 模板匹配置信度阈值（0.80兼顾准确率和召回率）
        # 热键跑马灯滚动偏移（从右到左流动）
    ...
    def _ensure_ui_window_size(self):
        """检测控制面板窗口大小是否变动，变动则拉回固定大小1382x807（游戏窗口尺寸）"""
        if not getattr(self, '_win_name', None):
            return  # 窗口未创建，跳过
        try:
            # 获取窗口当前客户区大小
            hwnd_ui = user32.FindWindowW(None, self._win_name)
            if hwnd_ui:
                rect = ctypes.wintypes.RECT()
                user32.GetClientRect(hwnd_ui, ctypes.byref(rect))
                cur_w = rect.right - rect.left
                cur_h = rect.bottom - rect.top
                FIX_W = 1382  # 游戏窗口宽度（记忆文件中记录的固定值）
                FIX_H = 807   # 游戏窗口高度（记忆文件中记录的固定值）
                if abs(cur_w - FIX_W) > 2 or abs(cur_h - FIX_H) > 2:
                    cv2.resizeWindow(self._win_name, FIX_W, FIX_H)
                    print("[UI窗口固定] 检测到大小变动 %dx%d -> 已拉回 %dx%d" % (cur_w, cur_h, FIX_W, FIX_H))
        except Exception as _e:
            print("[UI窗口固定] 检测异常:", _e)

    def _ensure_ui_window_size(self):
        """检测控制面板(UI)窗口大小是否变动，变动则拉回固定大小1382x807（游戏窗口尺寸）"""
        if not getattr(self, '_win_name', None):
            return  # 窗口未创建，跳过
        try:
            # 获取窗口当前客户区大小
            ...
            if hwnd_ui:
                ...
                cur_w = rect.right - rect.left
                cur_h = rect.bottom - rect.top
                UI_LOCK_W = 1382  # UI窗口锁定宽度（游戏窗口尺寸，来源：记忆文件）
                UI_LOCK_H = 807   # UI窗口锁定高度（游戏窗口尺寸，来源：记忆文件）
                if abs(cur_w - UI_LOCK_W) > 2 or abs(cur_h - UI_LOCK_H) > 2:
                    cv2.resizeWindow(self._win_name, UI_LOCK_W, UI_LOCK_H)
                    print("[UI窗口固定] 检测到大小变动 %dx%d -> 已拉回 %dx%d" % (cur_w, cur_h, UI_LOCK_W, UI_LOCK_H))
        except Exception as _e:
            print("[UI窗口固定] 检测异常:", _e)
    ...
    def _detect_minimap(self, debug=True):
        ...
        LEFT_OFFSET = 6  # 左边增加6像素白边（和右边一样宽）
    ...
    def find_player_dot(self, map_area):
        """【模块B】检测小地图上人物黄色光点（改进版：取最亮区域内的最亮像素作为中心点）
        原理：HSV颜色过滤找黄色区域，对每个轮廓计算平均亮度，取最亮的轮廓，然后在该轮廓内取最亮像素作为中心点（人物光点最亮部位）
        返回：(x, y) 光点最亮部位坐标；找不到返回None"""
        hsv = cv2.cvtColor(map_area, cv2.COLOR_BGR2HSV)  # 转换到HSV颜色空间
        lower = np.array([YELLOW_H_LOW, YELLOW_S_LOW, YELLOW_V_LOW])  # 黄色下限（收紧后）
        upper = np.array([YELLOW_H_HIGH, 255, 255])  # 黄色上限
        h, w = map_area.shape[:2]  # 获取小地图尺寸
        mask = cv2.inRange(hsv, lower, upper)  # 黄色过滤mask
        # mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((2, 2), np.uint8))  # 注释掉MORPH_OPEN，避免吃掉2~4px的小光点
        cnts, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # 找所有黄色轮廓
        if cnts:
            # 对每个轮廓计算质心和平均亮度，取最亮的（人物光点最亮，不靠面积过滤）
            candidates = []
            v_channel = hsv[:, :, 2]  # V通道（亮度），用于计算轮廓平均亮度
            for c in cnts:
                area = cv2.contourArea(c)  # 轮廓面积
                if area <= 0:
                    continue  # 跳过面积为0的无效轮廓
                M = cv2.moments(c)  # 计算轮廓矩
                if M["m00"] > 0:
                    cx = int(M["m10"] / M["m00"])  # 质心X
                    cy = int(M["m01"] / M["m00"])  # 质心Y
                    # 计算该轮廓区域的平均亮度（V值），人物光点最亮
                    contour_mask = np.zeros(mask.shape, dtype=np.uint8)  # 创建单轮廓mask
                    cv2.drawContours(contour_mask, [c], -1, 255, -1)  # 填充轮廓
                    mean_val = cv2.mean(v_channel, mask=contour_mask)[0]  # 计算平均亮度
                    candidates.append((cx, cy, area, mean_val, contour_mask))  # 保存候选：质心+面积+平均亮度+轮廓mask
            if candidates:
                if getattr(self, 'frame_count', 0) % 10 == 0:
                    _debug_log("[光点检测] 候选数=%d candidates=%s" % (len(candidates), [(c[0], c[1], c[2], c[3]) for c in candidates]))  # 调试日志：每10帧输出候选轮廓
                # 取平均亮度最高的轮廓（人物光点最亮，去掉跟踪逻辑避免匹配到固定物体）
                best = max(candidates, key=lambda p: p[3])
                best_cx, best_cy, best_area, best_mean, best_mask = best
                # 在最亮轮廓内，取最亮的像素作为中心点（人物光点最亮部位，比质心更准确）
                best_v_region = np.where(best_mask > 0, v_channel, 0)  # 只保留轮廓内的亮度值
                max_v = np.max(best_v_region)  # 轮廓内最亮的亮度值
                brightest_ys, brightest_xs = np.where(best_v_region == max_v)  # 找到最亮像素的坐标
                if len(brightest_xs) > 0:
                    # 取最亮像素的质心（如果有多个最亮像素，取平均位置）
                    final_x = int(np.mean(brightest_xs))
                    final_y = int(np.mean(brightest_ys))
                    self.last_player_pos = (final_x, final_y)  # 更新上次位置
                    return (final_x, final_y)  # 返回最亮像素的位置
                else:
                    # 如果找不到最亮像素，回退到轮廓质心
                    self.last_player_pos = (best_cx, best_cy)  # 更新上次位置
                    return (best_cx, best_cy)  # 返回轮廓质心
    ...
    def _screen_to_map(self, screen_x, screen_y):
        ...
        # X和Y用各自的scale（小地图X/Y压缩比率不同，必须分开算）
        # scale_x靠左右端点校准最准，scale_y靠上端点校准或人物上下移动自动校准
        # 实时计算总倍率=基础倍率+偏差（偏差输入后立即生效，不需要保存配置）
        base_sx = getattr(self, '_calibrated_scale_x', 0.10)
        base_sy = getattr(self, '_calibrated_scale_y', 0.10)
        try:
            offset_sx = float(self._field_values.get("scale_x_offset", "0") or "0")
            offset_sy = float(self._field_values.get("scale_y_offset", "0") or "0")
        except Exception:
            offset_sx = 0.0
            offset_sy = 0.0
        scale_x = base_sx + offset_sx  # 最终X倍率=基础+偏差
        scale_y = base_sy + offset_sy  # 最终Y倍率=基础+偏差
    ...
    def _start_auto_calibration(self):
        """【模块B】自动校准（五步流程，每次点倍率按钮推进一步）
        第1次点(0→1)：记录基点，蒙板出绿蓝圆，可蒙板拖动到特色背景位置
        第2次点(1→2)：截图绿蓝圆位置的背景图保存为模板，开始模板匹配跟踪，蒙板画绿/蓝空心圆
        第3次点(2→3)：人物走到绿色圆上，记录绿点小地图坐标（人物光点位置）
        第4次点(3→4)：人物走到蓝色圆上，记录蓝点小地图坐标
        第5次点(4→0)：计算倍率（X/Y分开），关闭所有显示，清空点位，保存倍率"""
        # 阶段4→0：计算倍率，关闭显示，清空点位和模板，保存倍率
        if self._auto_calib_stage == 4:
            ...
            self._add_log("【校准完成】倍率已保存，点位已清空，显示已关闭")
            return
        # 【关键】每次点倍率记录时，强制重新截取小地图+检测人物光点，确保用最新准确坐标（不用缓存）
        if self._auto_calib_stage in (0, 1, 2, 3):
            try:
                _map_frame = self._capture_map()
                if _map_frame is not None:
                    _detected = self.find_player_dot(_map_frame)
                    _debug_log("[校准] 重新检测人物光点: stage=%d detected=%s old_pos=%s" % (self._auto_calib_stage, str(_detected), str(self._player_map_pos)))  # 调试日志：验证重新检测是否更新了人物光点
                    if _detected:
                        self._player_map_pos = _detected
                        self._add_log("【校准】重新检测人物光点: 小地图(%d,%d)" % (_detected[0], _detected[1]))
            except Exception as _e:
                _debug_log("[校准] 重新检测人物光点失败: %s" % _e)
        # 人物位置检查：stage 0→1/1→2需要屏幕+小地图坐标；stage 2→3/3→4只需要小地图坐标
        cur_sx = cur_sy = cur_mx = cur_my = 0
        if self._auto_calib_stage in (0, 1):
            if not self._player_map_pos or not self._player_screen_pos:
                self._add_log("自动校准失败：未检测到人物位置，请确保人物在屏幕内")
                return
            ...
            if not self._player_map_pos:
                self._add_log("自动校准失败：未检测到小地图人物光点")
                return
            cur_mx, cur_my = self._player_map_pos[0], self._player_map_pos[1]
        # 阶段0→1：开始校准（先清空旧数据再录入）
        if self._auto_calib_stage == 0:
            ...
            self._calib_green_match_pos = None
            self._calib_blue_match_pos = None
            # stage=1不固定基点，基点在主循环中实时跟随人物移动
            # 绿点蓝点用相对偏移存储，默认右800/上400，用户可蒙板拖动调整
            self._auto_calib_green_offset = (400, 0)
            self._auto_calib_blue_offset = (0, -400)
            self._auto_calib_stage = 1
            self._add_log("【第1步/5】请移动人物找特色位置，红色基点跟随人物移动，拖动绿点蓝点调整位置，定好后点倍率")
            return
        # 阶段1→2：固定基点（记录当前人物位置），用偏移计算绿点蓝点屏幕坐标，截图模板
        if self._auto_calib_stage == 1:
            # 固定基点：记录当前人物屏幕坐标+小地图坐标
            self._auto_calib_base = (cur_sx, cur_sy, cur_mx, cur_my)
            # 用相对偏移计算绿点蓝点屏幕坐标（固定下来，stage>=2不再跟随人物）
            ...
            # 截图前隐藏蒙板，避免截到绿点蓝点本身（模板必须是纯背景特色）
            if self._overlay_hwnd:
                user32.ShowWindow(self._overlay_hwnd, 0)  # SW_HIDE
                time.sleep(0.1)  # 等待窗口完全隐藏生效
            # 截图绿点位置的背景模板（加延时错开1秒，避免连续截图冲突）
            self._capture_calib_template(self._auto_calib_green_screen, 'green')
            time.sleep(1.0)  # 错开截图1秒，避免mss连续调用冲突
            # 截图蓝点位置的背景模板
            self._capture_calib_template(self._auto_calib_blue_screen, 'blue')
            time.sleep(0.05)  # 等待截图完成
            # 恢复蒙板显示
            if self._overlay_hwnd:
                user32.ShowWindow(self._overlay_hwnd, 5)  # SW_SHOW
                time.sleep(0.05)  # 等待窗口显示生效
            self._auto_calib_stage = 2
            self._add_log("【第2步/5】基点已固定(屏幕%d,%d 小地图%d,%d)，模板截图完成，开始跟踪。请人物走到绿色圆上，站好后点倍率" % (
                cur_sx, cur_sy, cur_mx, cur_my))
            return
        # 阶段2→3：人物走到绿圆上，记录绿点小地图坐标
        if self._auto_calib_stage == 2:
            _debug_log("[校准] 记录绿点: cur_mx=%d cur_my=%d player_map_pos=%s" % (cur_mx, cur_my, str(self._player_map_pos)))  # 调试日志：记录绿点时的人物光点坐标
            self._auto_calib_green_map = (cur_mx, cur_my)
            self._auto_calib_stage = 3
            self._add_log("【第3步/5】绿点已记录(小地图%d,%d)，请人物走到蓝色圆上，站好后点倍率" % (cur_mx, cur_my))
            return
        # 阶段3→4：人物走到蓝圆上，记录蓝点小地图坐标
        if self._auto_calib_stage == 3:
            _debug_log("[校准] 记录蓝点: cur_mx=%d cur_my=%d player_map_pos=%s green_map=%s" % (cur_mx, cur_my, str(self._player_map_pos), str(self._auto_calib_green_map)))  # 调试日志：记录蓝点时的人物光点坐标和绿点坐标对比
            self._auto_calib_blue_map = (cur_mx, cur_my)
            self._auto_calib_stage = 4
            self._add_log("【第4步/5】蓝点已记录(小地图%d,%d)，再点一次倍率计算倍率并关闭" % (cur_mx, cur_my))
            return

    def _finish_auto_calibration(self):
        """【模块B】完成自动校准：用实际屏幕距离和小地图距离计算scale_x和scale_y"""
        if not self._auto_calib_base or not self._auto_calib_green_map or not self._auto_calib_blue_map:
            self._add_log("自动校准失败：数据不完整")
            return
        if not self._auto_calib_green_screen or not self._auto_calib_blue_screen:
            self._add_log("自动校准失败：屏幕位置不完整")
            return
        ...
        # 实际屏幕距离（蒙板拖动后的值，不是固定800/400）
        dx_screen = green_sx - base_sx
        dy_screen = base_sy - blue_sy
        # 小地图距离（人物走到特色位置后记录的光点位置差）
        dx_map = green_mx - base_mx
        dy_map = base_my - blue_my
        if dx_screen <= 0 or dx_map <= 0:
            self._add_log("自动校准失败：绿点应在基点右方，屏幕距离=%d 小地图距离=%d" % (dx_screen, dx_map))
            return
        if dy_screen <= 0 or dy_map <= 0:
            self._add_log("自动校准失败：蓝点应在基点上方，屏幕距离=%d 小地图距离=%d" % (dy_screen, dy_map))
            return
        scale_x = (dx_map / 2.0) / float(dx_screen)  # 蒙板是小地图的二倍，除以2转换成原始小地图距离
        scale_y = (dy_map / 2.0) / float(dy_screen)  # 蒙板是小地图的二倍，除以2转换成原始小地图距离
        self._calibrated_scale_x = scale_x
        self._calibrated_scale_y = scale_y
        self._map_screen_scale = scale_x
        # 注意：不再设置stage=3，由_start_auto_calibration在stage=4→0时调用本函数，调用后直接清空并关闭
        self._add_log("校准完成：scale_x=%.4f (小地图%dpx/屏幕%dpx) scale_y=%.4f (小地图%dpx/屏幕%dpx)" % (
    ...
    def _on_mouse(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONUP:
            ...
            # 结束弹窗拖拽
            if self._scale_dialog_dragging:
                self._scale_dialog_dragging = False
                print("[倍率差弹窗] 结束拖拽，位置:", self._scale_dialog_pos)
        # 鼠标移动：拖拽弹窗
        if event == cv2.EVENT_MOUSEMOVE and self._scale_dialog_dragging:
            new_x = x - self._scale_dialog_drag_offset[0]
            new_y = y - self._scale_dialog_drag_offset[1]
            # 边界保护，确保弹窗不会拖出窗口
            new_x = max(0, min(new_x, 461 - 320))
            new_y = max(0, min(new_y, 900 - 220))
            self._scale_dialog_pos = [new_x, new_y]
            return
        ...
        # === 【模块B】倍率差调整弹窗点击处理 ===
        if event == cv2.EVENT_LBUTTONDOWN:
            # 点击右上角"倍率差"按钮：打开/关闭弹窗
            btn_sx, btn_sy, btn_sw, btn_sh = self._btn_scale_dialog
            if btn_sx <= x < btn_sx + btn_sw and btn_sy <= y < btn_sy + btn_sh:
                self._show_scale_dialog = not self._show_scale_dialog
                if self._show_scale_dialog:
                    # 打开弹窗时备份原始值（确认才保存，取消/关闭恢复）
                    ...
                    self._update_scale_dialog_positions()  # 打开弹窗时立即计算所有控件位置，解决第一次点击没反应的问题
                if not self._show_scale_dialog:
                    self._focused_field = None
                print("[倍率差弹窗] 打开" if self._show_scale_dialog else "[倍率差弹窗] 关闭")
                return
            # 如果弹窗打开，处理弹窗内的点击
            if self._show_scale_dialog:
                # 点击右上角关闭按钮X：关闭弹窗（不保存，恢复原始值，优先检测，因为X在标题栏区域）
                close_x, close_y, close_w, close_h = self._dlg_scale_close_btn
                if close_x <= x < close_x + close_w and close_y <= y < close_y + close_h:
                    # 恢复原始值
                    self._field_values["scale_x_offset"] = self._scale_dialog_backup.get("scale_x_offset", "0")
                    self._field_values["scale_y_offset"] = self._scale_dialog_backup.get("scale_y_offset", "0")
                    self._show_scale_dialog = False
                    self._focused_field = None
                    print("[倍率差弹窗] 点击X关闭，恢复原始值")
                    return
                # 点击标题栏：开始拖拽弹窗（标题栏高度50像素）
                dlg_x = self._scale_dialog_pos[0]
                dlg_y = self._scale_dialog_pos[1]
                if dlg_x <= x < dlg_x + 320 and dlg_y <= y < dlg_y + 50:
                ...
                if cancel_x <= x < cancel_x + cancel_w and cancel_y <= y < cancel_y + cancel_h:
                    ...
                    self._show_scale_dialog = False
                    self._focused_field = None
                    print("[倍率差弹窗] 取消，恢复原始值")
                    return
                # 点击弹窗外部：关闭弹窗（使用弹窗位置变量）
                dlg_x = self._scale_dialog_pos[0]
                dlg_y = self._scale_dialog_pos[1]
                dlg_w, dlg_h = 320, 220
        ...
        # 5. 小地图区域内点击（修复：点击检测区域=实际绘制区域 60,143,340,250，不拉伸）
        if 60 <= x < 400 and 143 <= y < 393:
            # UI坐标转小地图原始分辨率坐标
            map_w = getattr(self, '_last_map_w', FIXED_W)
            map_h = getattr(self, '_last_map_h', MAP_H)
            map_x = int((x - 60) / 340 * map_w)
            map_y = int((y - 143) / 250 * map_h)
            # 【模块B】台子选择按钮点击（小地图左上方）
    ...
    def draw(self, map_area, player_pos):
        ...
        # 绿点：stage=3（已记录绿点）和stage=4（已记录蓝点）时显示
        ...
        # 【模块B】自动校准倍率显示（移到右上角，倍率差输入框上方，画在frame上）
        # （原代码在小地图底部显示，已移到下面frame合成之后）
        ...
        # 【模块B】自动校准按钮（同屏三点校准，用背景图片自带按钮，只保留点击功能，不绘制UI）
        # draw_asset(frame, self._ui_calib_auto, *BTN_CALIB_AUTO)  # 去掉UI绘制，用背景图片自带按钮
        # 【模块B】按钮按下阴影特效（和前面按钮一样的效果）
        ...
        ply = BTN_PLAN_TOOLBAR[1] + BTN_PLAN_TOOLBAR[3] // 2 + plh // 2  # 文字垂直居中
        cv2.putText(frame, plan_label, (plx, ply), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)

        # === 【模块B】倍率差调整按钮（用背景图片自带的按钮，只保留点击事件，不绘制UI）===
        # 按钮位置由self._btn_scale_dialog定义，点击事件在_on_mouse中处理
        ...
        # 【模块B】自动校准倍率显示（小地图下方，红色加粗放大，外面包白边）
        calib_sx = getattr(self, '_calibrated_scale_x', 0)
        calib_sy = getattr(self, '_calibrated_scale_y', 0)
        if calib_sx > 0 and calib_sy > 0:
            scale_text = "倍率 X:%.4f Y:%.4f" % (calib_sx, calib_sy)
            # 计算文字位置（小地图下方居中）
            (tw, th), _ = cv2.getTextSize(scale_text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            tx = map_display_x + (map_w - tw) // 2
            ty = map_display_y + map_h + th + 8
            # 外面包一圈白边（先画白色粗描边，再画红色文字）
            cv2.putText(frame, scale_text, (tx, ty), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 4, cv2.LINE_AA)  # 白色描边
            cv2.putText(frame, scale_text, (tx, ty), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2, cv2.LINE_AA)  # 红色加粗放大
        ...
        # 在怪物数据按钮右侧白色区域显示文件夹名+BEST.ONNX（自动换行，最多2行）【恢复动态信息绘制】
        ...
        draw_asset(frame, self._ui_winbind_bg, *BTN_WINBIND)  # 【保留窗口绑定功能】
        # 已绑定窗口下拉框
        draw_asset(frame, self._ui_bound_dropdown, UI_BOUND_X, UI_BOUND_Y, UI_BOUND_W, UI_BOUND_H)  # 【保留窗口绑定功能】

        # === 录制状态红色闪烁指示器（在对应按钮左上角）=== 【已去掉UI绘制】
        # import time as _t
        ...
        # === 下拉菜单 === 【恢复动态下拉菜单绘制，保存/方案/模式/清除方案按钮点击后显示】
        if self._dropdown is not None:
            items = self._dropdown_items()
            n = len(items)
            dd_btn_map = {"save": BTN_SAVE, "route": BTN_PLAN, "mode": BTN_MODE, "clear_route": BTN_PLAN_CLR}
        ...
        # === 运行日志区域（日志底板+向上流动+右侧滚动条）=== 【已去掉UI绘制】
        ...
        # === 人物特征下拉面板（向下弹出，5行：删除全部+4特征）=== 【恢复动态信息绘制】
        if self._char_dropdown:
            dd_top = BTN_CHAR[1] + BTN_CHAR[3]
            dd_bottom = dd_top + CHAR_DD_VISIBLE * CHAR_DD_ITEM_H
            dd_main_x2 = CHAR_DD_X + CHAR_DD_W
        ...
        # === 路线页输入框（X/Y偏移，标签下方）=== 【恢复动态信息绘制】
        self._draw_input_fields(frame)

        # === 按钮点击特效（仅按下变暗，圆角）=== 【已去掉UI绘制】
        # now_ms = time.time() * 1000
        ...
        # === 【模块B】倍率差调整弹窗（灰底白字，简单明了，可拖拽移动，放在最上层不被挡住）===
        if self._show_scale_dialog:
            # 弹窗半透明背景（灰色，位置可移动）
            dlg_x = self._scale_dialog_pos[0]
            dlg_y = self._scale_dialog_pos[1]
            dlg_w, dlg_h = 320, 220
            overlay = frame.copy()
            cv2.rectangle(overlay, (dlg_x, dlg_y), (dlg_x+dlg_w, dlg_y+dlg_h), (60, 60, 60), -1)
            cv2.addWeighted(overlay, 0.95, frame, 0.05, 0, frame)
            cv2.rectangle(frame, (dlg_x, dlg_y), (dlg_x+dlg_w, dlg_y+dlg_h), (120, 120, 120), 2)
            # 标题（白色文字）
            cv2.putText(frame, "倍率差调整", (dlg_x+100, dlg_y+35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
            # 右上角关闭按钮X（深灰背景白色X，点击关闭弹窗）
            close_x, close_y, close_w, close_h = dlg_x+dlg_w-30, dlg_y+5, 25, 25
            cv2.rectangle(frame, (close_x, close_y), (close_x+close_w, close_y+close_h), (80, 80, 80), -1)
            cv2.rectangle(frame, (close_x, close_y), (close_x+close_w, close_y+close_h), (180, 180, 180), 1)
            cv2.putText(frame, "X", (close_x+7, close_y+18), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2, cv2.LINE_AA)
            self._dlg_scale_close_btn = (close_x, close_y, close_w, close_h)  # 保存关闭按钮位置供点击检测
            # X偏差标签和输入框（白色标签）
            cv2.putText(frame, "X偏差：", (dlg_x+30, dlg_y+85), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1, cv2.LINE_AA)
            sx_x, sx_y, sx_w, sx_h = dlg_x+110, dlg_y+65, 160, 35
            cv2.rectangle(frame, (sx_x, sx_y), (sx_x+sx_w, sx_y+sx_h), (0, 0, 0), -1)  # 黑底
            cv2.rectangle(frame, (sx_x, sx_y), (sx_x+sx_w, sx_y+sx_h), (255, 255, 255), 1)  # 白边框
            if self._focused_field == "scale_x_offset":
                cv2.rectangle(frame, (sx_x, sx_y), (sx_x+sx_w, sx_y+sx_h), (0, 165, 255), 2)
            sx_val = self._field_values.get("scale_x_offset", "")
            if sx_val:
                (sx_tw, sx_th), _ = cv2.getTextSize(sx_val, cv2.FONT_HERSHEY_SIMPLEX, 0.55, 1)
                cv2.putText(frame, sx_val, (sx_x+(sx_w-sx_tw)//2, sx_y+(sx_h+sx_th)//2-2), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1, cv2.LINE_AA)  # 白字
            # Y偏差标签和输入框（白色标签）
            cv2.putText(frame, "Y偏差：", (dlg_x+30, dlg_y+145), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1, cv2.LINE_AA)
            sy_x, sy_y, sy_w, sy_h = dlg_x+110, dlg_y+125, 160, 35
            cv2.rectangle(frame, (sy_x, sy_y), (sy_x+sy_w, sy_y+sy_h), (0, 0, 0), -1)  # 黑底
            cv2.rectangle(frame, (sy_x, sy_y), (sy_x+sy_w, sy_y+sy_h), (255, 255, 255), 1)  # 白边框
            if self._focused_field == "scale_y_offset":
                cv2.rectangle(frame, (sy_x, sy_y), (sy_x+sy_w, sy_y+sy_h), (0, 165, 255), 2)
            sy_val = self._field_values.get("scale_y_offset", "")
            if sy_val:
                (sy_tw, sy_th), _ = cv2.getTextSize(sy_val, cv2.FONT_HERSHEY_SIMPLEX, 0.55, 1)
                cv2.putText(frame, sy_val, (sy_x+(sy_w-sy_tw)//2, sy_y+(sy_h+sy_th)//2-2), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1, cv2.LINE_AA)  # 白字
            # 确认按钮（灰色背景白色文字）
            ok_x, ok_y, ok_w, ok_h = dlg_x+60, dlg_y+175, 80, 30
            cv2.rectangle(frame, (ok_x, ok_y), (ok_x+ok_w, ok_y+ok_h), (80, 80, 80), -1)
            cv2.rectangle(frame, (ok_x, ok_y), (ok_x+ok_w, ok_y+ok_h), (180, 180, 180), 1)
            cv2.putText(frame, "确认", (ok_x+20, ok_y+22), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
            # 取消按钮（深灰色背景白色文字）
            cancel_x, cancel_y, cancel_w, cancel_h = dlg_x+180, dlg_y+175, 80, 30
            cv2.rectangle(frame, (cancel_x, cancel_y), (cancel_x+cancel_w, cancel_y+cancel_h), (50, 50, 50), -1)
            cv2.rectangle(frame, (cancel_x, cancel_y), (cancel_x+cancel_w, cancel_y+cancel_h), (180, 180, 180), 1)
            cv2.putText(frame, "取消", (cancel_x+20, cancel_y+22), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
            # 保存弹窗按钮位置供点击检测
            self._dlg_scale_x_input = (sx_x, sx_y, sx_w, sx_h)
            self._dlg_scale_y_input = (sy_x, sy_y, sy_w, sy_h)
            self._dlg_scale_ok_btn = (ok_x, ok_y, ok_w, ok_h)
            self._dlg_scale_cancel_btn = (cancel_x, cancel_y, cancel_w, cancel_h)
    ...
    def _update_scale_dialog_positions(self):
        """立即计算倍率差弹窗内所有控件的位置（解决第一次打开弹窗点击没反应的问题）
        位置变量在弹窗绘制时才设置，但第一次打开弹窗时draw还没执行到最后，导致点击检测失败
        所以在打开弹窗时立即调用此方法，提前计算所有位置变量"""
            ...
            try:
                ...
                # === 自动校准蒙板拖动（仅stage=1时，拖动绿点蓝点定特色位置）===
                    ...
                    try:
                        ...
                        if auto_stage >= 1:
                            step_texts = {
                                1: "【第1步/5】拖动绿蓝圆到特色背景位置，定好后点倍率",
                                2: "【第2步/5】人物走到绿色圆上，站好后点倍率",
                                3: "【第3步/5】人物走到蓝色圆上，站好后点倍率",
                                4: "【第4步/5】再点一次倍率，计算倍率并关闭显示",
                            }
    ...
    def _save_input_config(self):
        """保存 _field_values 到磁盘（只保存已知字段且非空的值），倍率偏差不保存，总倍率保存到方案文件"""
        # 倍率字段：基础倍率保存，偏差不保存（偏差每次打开程序重新输入）
        scale_offset_ids = {"scale_x_offset", "scale_y_offset"}
        known_ids = set(f[5] for f in FIGHT_FIELDS + POTION_FIELDS + ROUTE_FIELDS)
        to_save = {k: v for k, v in self._field_values.items() if k in known_ids and v and k not in scale_offset_ids}
        try:
            with open(INPUT_CONFIG_FILE, "w", encoding="utf-8") as fp:
                json.dump(to_save, fp, ensure_ascii=False, indent=2)
        except Exception as e:
            print("[输入框] 保存配置失败:", e)
        # 自动计算总倍率=基础倍率+偏差，保存到方案文件（偏差不保存，总倍率保存）
        try:
            sx_base = float(self._field_values.get("scale_x_base", "0") or "0")
            sx_offset = float(self._field_values.get("scale_x_offset", "0") or "0")
            sy_base = float(self._field_values.get("scale_y_base", "0") or "0")
            sy_offset = float(self._field_values.get("scale_y_offset", "0") or "0")
            if sx_base > 0 and sy_base > 0:
                final_sx = sx_base + sx_offset  # 最终X倍率=基础+偏差
                final_sy = sy_base + sy_offset  # 最终Y倍率=基础+偏差
                self._calibrated_scale_x = final_sx
                self._calibrated_scale_y = final_sy
                self._map_screen_scale = final_sx
                self._save_calib()  # 保存总倍率到方案文件
                print("[倍率] 已计算并保存总倍率: X=%.4f (%.4f%+.4f) Y=%.4f (%.4f%+.4f)" % (
                    final_sx, sx_base, sx_offset, final_sy, sy_base, sy_offset))
        except Exception as e:
            print("[倍率] 计算总倍率失败:", e)
    ...
    def _handle_input_key(self, key):
        ...
        # 找字段类型（包含ROUTE_FIELDS，因为倍率差输入框在ROUTE_FIELDS中）
        ftype = "num"
        for f in FIGHT_FIELDS + POTION_FIELDS + ROUTE_FIELDS:
            if f[5] == fid:
                ftype = f[4]
                break
        ...
        for f in fields:
            ...
            if val:
                if fid in ("char_x_offset", "char_y_offset"):
                    ...
                    # 倍率输入框使用稍小的字体，让4位小数能显示完整（但不能太小，要能看清楚）
                    if fid in ("scale_x_base", "scale_x_offset", "scale_y_base", "scale_y_offset"):
                        fscale = 0.38
                        fthick = 1
                    else:
    ...
    def _poll_num_input(self):
        ...
        # 轮询：主键盘0-9(0x30-0x39) + 小键盘0-9(0x60-0x69) + 退格(0x08) + 回车(0x0D) + ESC(0x1B) + 负号(0xBD,0x6D) + 小数点(0xBE,0x6E)
        ...
        if 0x30 <= vk <= 0x39:
            ...
            # minus key - toggle negative sign only for offset fields（包含倍率差字段）
            ...
            if len(new_val) <= 10:
                ...
                self._last_input_change = time.time() * 1000
                # 【日志】记录小数点输入
                print("[小数点输入] fid=%s new_val=%s" % (fid, new_val))
            return
        else:
            return
        # 数字输入：首次覆盖，后续追加
        ...
        if len(new_val) <= 10:
            ...
            self._last_input_change = time.time() * 1000
            # 【日志】记录数字输入，方便排查问题
            print("[数字输入] fid=%s digit=%s new_val=%s" % (fid, digit, new_val))
            if fid in ("char_x_offset", "char_y_offset"):
                self._offset_feedback_start = time.time() * 1000
                self._offset_feedback_done = False
    ...
    def run(self):
        ...
        cv2.resizeWindow(win, 1382, 807)  # 显式设置固定窗口大小（游戏窗口尺寸，来源：记忆文件），加载脚本后先锁定
        cv2.setMouseCallback(win, self._on_mouse)
        self._win_name = win
        self._win_size = (UI_W, UI_H)
        print("[UI窗口固定] 初始大小设置为 1382x807")
        while True:
            try:
            ...
            if self.frame_count % 30 == 0:
                self._ensure_window_size()  # 固定游戏窗口大小
                self._ensure_ui_window_size()  # 固定控制面板窗口大小
            ...
            if self._auto_calib_stage == 1:
                ...
                if left_down and not self._auto_calib_dragging:
                    ...
                    # 拖动中，更新相对偏移（绿点只水平，蓝点只垂直，基点跟随人物）
                    if base:
                        bx, by = base[0], base[1]
                        if self._auto_calib_dragging == 'green':
                            self._auto_calib_green_offset = (mx - bx, 0)  # 只水平
                        elif self._auto_calib_dragging == 'blue':
                            self._auto_calib_blue_offset = (0, my - by)  # 只垂直
            ...
            if self._drag_crosshair:
                left_down = user32.GetAsyncKeyState(0x01) & 0x8000  # VK_LBUTTON
                if left_down:
                    # 跟随全局鼠标位置，但准星显示限制在UI窗口内（避免拖出窗口看不到）
                    cursor = POINT()
                    ...
                    if hwnd_ui:
                        ...
                        # 限制准星在UI窗口内，避免拖出窗口看不到
                        cx = max(self._crosshair_size // 2, min(UI_W - self._crosshair_size // 2, cursor.x))
                        cy = max(self._crosshair_size // 2, min(UI_H - self._crosshair_size // 2, cursor.y))
                        self._crosshair_pos = (cx, cy)
                else:
                    # 左键释放，绑定鼠标指向的顶层窗口
                    cursor = POINT()
                    user32.GetCursorPos(cursor)
            ...
            # 输入框自动失焦：5秒无变化（全局轮询输入不依赖UI前台，故不检查前台窗口）
            if self._focused_field is not None:
                now_ms = time.time() * 1000
