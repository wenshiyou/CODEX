...
class MinimapRouteRecorder:
    ...
    def _is_mp_label_visible(self, frame):
        ...
        if not visible:
            _debug_log("[MP遮挡] 标签匹配度%.3f<0.75, 判定被遮挡(底部80px全宽)" % max_val)
        else:
            self._mp_label_pos = (max_loc[0], y_start + max_loc[1])  # 保存标签位置供蒙板显示
        return visible
