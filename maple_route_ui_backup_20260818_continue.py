...
class MinimapRouteRecorder:
    def __init__(self):
        ...
        self._combat_timed_keys = []       # 定时释放的按键 [(vk, release_ms)]
    ...
    def _is_bar_blank_at(self, frame, bar, pct, color_type):
        ...
        简单模式：只看竖框内有没有填充色，过半没有则判定为空白（血量低于阈值）"""
    ...
    def _detect_monster_hp_bars(self, frame, search_areas=None):
        ...
        # 绿色（HP高时）
        m_g = cv2.inRange(hsv, np.array([35, 60, 60]), np.array([85, 255, 255]))
    ...
    def _check_auto_potion(self):
        ...
        # MP遮挡检测：MP标签不可见则跳过吃药
        mp_occluded = not self._is_mp_label_visible(frame)
    ...
    def _combat_tick(self):
        ...
        if self._combat_timed_keys:
            ...
            for _vk, _rel in self._combat_timed_keys:
                if now >= _rel:
                    user32.keybd_event(_vk, 0, 2, 0)
                else:
                    _rem.append((_vk, _rel))
            self._combat_timed_keys = _rem

        # === YOLO怪物检测（每350ms一次）===
        if now - self._last_yolo_check > 250:
            ...
            if frame is not None:
                ...
                # === 怪物血条检测（近战挡住怪身体时凭血条定位）===
                if self._monsters:
                    _search = [(max(0, x1-15), max(0, y1-35), x2+15, y1+5)
                               for (x1, y1, x2, y2, _) in self._monsters]
                else:
                    _search = None
                self._monster_hp_bars = self._detect_monster_hp_bars(frame, _search)
                if not self._monsters and self._monster_hp_bars:
                    self._monsters = [(bx, by+bh+5, bx+bw, by+bh+50, 0.4)
                                      for (bx, by, bw, bh) in self._monster_hp_bars]
                    self._rlog("血条定位%d只" % len(self._monster_hp_bars), (0, 255, 0))

        # === 发呆状态（1-3秒，什么都不做）===
        if now < self._combat_idle_until:
            return
        if now - self._combat_last_idle_check > random.randint(15000, 30000):
            self._combat_last_idle_check = now
            if random.random() < 0.15:
                self._combat_idle_until = now + random.randint(500, 1500)
                self._rlog("（停顿）", (128, 128, 128))
                return
        ...
        if not has_target:
            ...
            if now - self._combat_last_move > random.randint(1500, 4000):
                self._combat_last_move = now
                if random.random() < 0.5:
                    _vk = random.choice([0x25, 0x27])
                    user32.keybd_event(_vk, 0, 0, 0)
                    self._combat_timed_keys.append((_vk, now + random.randint(80, 200)))
            if now - self._combat_last_jump > random.randint(4000, 10000):
                self._combat_last_jump = now
                if random.random() < 0.3:
                    self._press_game_key("alt", duration=random.randint(50, 100))
            return

        # === 有目标 ===
        px, py = self._player_screen_pos

        # 首次发现目标：反应延迟 150-500ms
        ...
        # 目标选择：70%最近，30%随机；每帧10%概率换目标
        if self._combat_target_idx >= len(monster_dists) or random.random() < 0.1:
            if random.random() < 0.7:
                self._combat_target_idx = 0
            else:
                self._combat_target_idx = random.randint(0, len(monster_dists) - 1)
        target = monster_dists[min(self._combat_target_idx, len(monster_dists) - 1)]
        ...
        # 攻击间隙走位（20%概率左右小步）
        if now - self._combat_last_move > random.randint(2000, 5000):
            self._combat_last_move = now
            if random.random() < 0.1:
                _vk = random.choice([0x25, 0x27])
                user32.keybd_event(_vk, 0, 0, 0)
                self._combat_timed_keys.append((_vk, now + random.randint(50, 150)))
                self._combat_busy_until = now + random.randint(80, 200)
                return

        # 攻击中偶尔跳（25%概率）
        if now - self._combat_last_jump > random.randint(5000, 12000):
            self._combat_last_jump = now
            if random.random() < 0.15:
                self._press_game_key("alt", duration=random.randint(50, 100))
                self._combat_busy_until = now + random.randint(150, 300)
                return
        ...
        if not skill_cast and aoe_key:
            ...
            if in_range >= 3 and now - last > aoe_cd + random.randint(-skill_rand, skill_rand):
                if random.random() < 0.8:
                    ...
                    skill_cast = True
                    self._combat_busy_until = now + random.randint(150, 400)
        ...
        if not skill_cast and atk_key:
            ...
            if t_dist <= atk_dist and now - last > atk_cd + random.randint(-skill_rand, skill_rand):
                ...
                skill_cast = True
                self._combat_busy_until = now + random.randint(100, 350)
