...
LADDER_TPL_ENTER_PX = 7        # 小地图|人-梯X差|≤此值才从"小地图粗导航"切到"主窗口梯子模板精对齐"(5→7放宽求稳:屏幕45~50跑跳带≈小地图4.5~5,确保跑跳带由屏幕逻辑接管)
...
class MinimapRouteRecorder:
    ...
    def _ladder_post_jump_process(self, py, now_ms):
        ...
        # ===== 跑跳专用(用户2026-09-11 22:38改动作):方向键不松按跳 → 跳完(=跳键duration80ms自动抬起)那一刻左右水平键全松、同时只按住↑;
        #       腾空头80ms仍保持朝梯水平键把助跑速度带进跳跃,跳完即收水平只留↑,靠起跳惯性滑到梯位抓梯(不再空中持续按水平、也不等贴脸才松) =====
        if step == 'run_delay':
            _rd = int(getattr(self, '_ladder_run_dir', 1) or 1)
            _hvk = VK_RIGHT if _rd > 0 else VK_LEFT
            _ovk = VK_LEFT if _rd > 0 else VK_RIGHT
            if now_ms - start_t < RUNJUMP_UP_DELAY_MS:
                # 腾空头80ms:只保持朝梯水平键(对侧先松防左右相抵),把助跑水平速度带进跳跃,这段先不按↑
                if _ovk in self._random_move_keys:
                    self._key_up(_ovk)
                if _hvk not in self._random_move_keys:
                    self._key_down(_hvk)                       # 保持朝梯=带水平惯性跃向梯子
                return False
            # 跳完(跳键80ms已自动抬起):跳键+左右水平键一起松,同时只按住↑不松
            self._key_up(VK_LEFT)
            self._key_up(VK_RIGHT)
            if VK_DOWN in self._random_move_keys:
                self._key_up(VK_DOWN)
            if VK_UP not in self._random_move_keys:
                self._key_down(VK_UP)
            self._ladder_post_jump_step = 'run_check'
            self._ladder_post_jump_t = now_ms
            _debug_log("[爬梯·屏幕·跑跳] 跳完%dms:左右水平一起松、同时只按住↑(靠起跳惯性滑到梯位),1秒内对比地面基准Y=%.0f(当前%.0f)" % (
                RUNJUMP_UP_DELAY_MS, self._climb_start_y, py))
            return False
        if step == 'run_check':
            # 跳完已全松水平:run_check只确保↑按住,绝不再按左右(保险再松一次,防残留水平键二次发力/惯性把人冲过梯子)
            if VK_LEFT in self._random_move_keys:
                self._key_up(VK_LEFT)
            if VK_RIGHT in self._random_move_keys:
                self._key_up(VK_RIGHT)
            ...
            if py < self._climb_start_y - LADDER_GRAB_UP_TOL:
                # Y变小=跑跳抓住:松水平(抓梯后按左右会横移/掉梯)、只留↑,转两步法持续爬到顶
                ...
                _ltrec = getattr(self, '_combat_locked_target', None)   # 跑跳抓住:冻结记录坐标点主窗口Y(到顶票2)
                ...
                _debug_log("[爬梯·屏幕·跑跳] 抓住梯子Y上升(→%.0f),松水平只留↑转持续爬+两步法到顶" % py)
                self._rlog("跑跳抓住梯子,持续向上到顶", LOG_OK, log='behavior')
                return False
            _run_el = now_ms - getattr(self, '_ladder_run_t0', start_t)
            # 用户2026-09-10:失败别发呆等满窗——过最短时间后,只要Y已落回起跳高度(抛物线回落=没抓住)立刻判失败;
            # 硬窗RUNJUMP_GRAB_WINDOW_MS仅兜底异常。抓住会先在上方触发Y变小锁存成功(不可逆),不会走到这。
            _run_fell_back = _run_el >= RUNJUMP_GRAB_FAIL_MIN_MS and py >= self._climb_start_y - 1
            if not _run_fell_back and _run_el < RUNJUMP_GRAB_WINDOW_MS:
                return False                               # 还在上升/腾空窗口内,继续保持朝梯+↑,镜头回弹帧忽略
            # 跑跳没抓住→【用户2026-09-10晚定稿:先跑跳、跑跳不上再贴脸直跳,不直接回主线】。走到这Y已回落=人已落地。
            # 不_reset_climb:保留_climb_state='to_ladder',钉死"跑跳已用掉"(段2不再重复跑跳)、开放唯一一次直跳;只清空中post_jump,
            # 下帧回到屏幕对位:35~70按住趋近→5~35三次碎步→≤5直跳(第2次起跳机会);那次直跳再失败由delay1/check分支回主线。
            _debug_log("[爬梯·屏幕·跑跳] 没抓住(%dms回落%.0f→%.0f),降级贴脸对齐直跳(第2次起跳机会,不回主线)" % (
                _run_el, self._climb_start_y, py))
            self._rlog("跑跳没挂上(%dms),改走到梯下碎步对齐+直跳一次" % _run_el, log='behavior')
            if VK_UP in self._random_move_keys:
                self._key_up(VK_UP)
            self._key_up(VK_LEFT)
            self._key_up(VK_RIGHT)
            self._ladder_run_jumped = True     # 跑跳机会已用掉,段2不再跑跳
            self._ladder_vert_jumped = False   # 开放后面唯一一次贴脸直跳
        ...
        if py < self._climb_start_y - LADDER_GRAB_UP_TOL:
            ...
            self._rlog("抓住梯子,持续向%s(梯端Y=%.0f 当前Y=%.0f)" % (
                ...
                log='behavior')
            return False
        # 用户2026-09-10定稿:失败别在梯下发呆5秒、不数连续次数、不退开横走(补丁越少越稳)。
        # 过最短时间LADDER_GRAB_FAIL_MIN_MS后,Y一旦落回起跳高度(跳了又落回原地=没抓住)立刻判失败回主线打怪;
        # 硬窗LADDER_GRAB_WINDOW_MS仅兜底异常。抓住会先在上方触发Y变小锁存成功(不可逆),不会误判。
        _el = now_ms - start_t
        ...
        if not _fell_back and _el < LADDER_GRAB_WINDOW_MS:
            ...
            return False
        _debug_log("[爬梯] 失败:起%dms Y未上升且已回落(%.0f→%.0f),松↑立刻回主线打怪(不计数/不退开)" % (
            _el, self._climb_start_y, py))
        self._rlog("起跳没抓住梯子(%dms已回落),回主线打怪" % _el, LOG_RED, log='behavior')
        if VK_UP in self._random_move_keys:
            self._key_up(VK_UP)
        self._climb_fail_pause_until = now_ms + LADDER_FAIL_REENTER_MS  # 极短固定冷却防同帧重选同梯空跳,不发呆
        self._reset_climb()
    ...
    def _descend_step(self, px, py, now_ms):
        ...
        if ph == 'check_drop':
            ...
            if _el >= DESC_DROP_CHECK_MS and _moved:
                ...
                # 整个观察窗Y都没增大=这位置是实心台真跳不下去,这才转方式二对齐梯子(用户:任意位置先直接跳,别动不动对齐)
                _debug_log("[下行·方式一] 观察%.0fms Y始终没增大(屏幕Δ%s/世界Δ%.0f)=真跳不下去,转方式二梯子" % (
                    _el, ("%.0f" % _dsy) if _dsy is not None else "NA", _dmy))
                self._rlog("直接跳%.0fms没下去,改走梯子" % _el, log='behavior')
                self._enter_desc_to_ladder(px, py, now_ms,
                                           reason="直接下跳观察窗满Y未增大(屏%d/世%d)" % (DESC_DROP_DY, DESC_DROP_DY_MAP))
    ...
    def _move_to(self, player_pos, target_x, target_y):
        ...
        if self._climb_state == "to_ladder":
            ...
            if _alx <= LADDER_TPL_ENTER_PX and getattr(self, '_ladder_templates', None) \
                ...
                self._ladder_precise_mode = True   # 已近梯切屏幕对位→检测线程进30Hz高频轻量档(方案B,出梯由_reset_climb关)
                _lk = getattr(self, '_combat_locked_target', None)
                _lkx = _lk[0] if _lk else None
                ...
                if _tpl_x is not None:
                    self._lad_scr_last_t = now_ms          # 记屏幕分段成功时刻/稳定X,供丢帧粘滞
                    self._lad_scr_last_x = _tpl_x
                    return self._ladder_align_by_screen(_tpl_x, px, py, now_ms, jump_key)
                # 模板偶发丢帧粘滞(用户2026-09-10):LADDER_SCR_STICK_MS内沿用上一次稳定梯X继续屏幕分段,
                # 不让单帧没匹配到就掉回小地图"走到X差0原地直跳",保证助跑→自适应跑跳状态连续
                _stick_x = getattr(self, '_lad_scr_last_x', None)
                if _stick_x is not None and now_ms - getattr(self, '_lad_scr_last_t', 0) <= LADDER_SCR_STICK_MS:
                    return self._ladder_align_by_screen(_stick_x, px, py, now_ms, jump_key)
            ...
            # ===== 水平对齐状态机(无屏幕模板回退路径;用户2026-09-10:全局小碎步只留屏幕直跳≤30一处,这里不再点动) =====
            # 没到容差就【持续按住】朝梯正常走(不短按点动、不磨叽);直跳是关键动作,必须|X差|≤1且连续2帧停稳才跳;
            # 持续按住但X一直没靠近(想动没动)按次数上限解卡回主线,不许卡边界死按/没对齐就跳。
            if _alx <= LADDER_ALIGN_TOL:
                _ak0 = getattr(self, '_lad_align_key_vk', None)  # 对准:抬起任何残留点动键+松左右
                if _ak0 is not None:
                    self._key_up(_ak0)
                ...
                if self._lad_align_ok_frames >= LADDER_ALIGN_HOLD_FRAMES and not getattr(self, '_ladder_vert_jumped', False):
                    ...
                    self._rlog("对齐重合直跳抓梯(X差%.1f),跳后50ms按↑" % ldx, log='behavior')
                    self._ladder_jump_phase = 'post_jump'
                    self._ladder_post_jump_step = 'delay1'
                    self._ladder_post_jump_t = now_ms
                return False
            # _alx>1:持续按住朝梯(正常移动,非点动);周期性检测"想动没动",超次数上限回主线不死磕
            self._lad_align_ok_frames = 0
            if now_ms - getattr(self, '_lad_align_nudge_t', 0) >= LADDER_ALIGN_STALL_MS:
            ...
            self._hold_toward_ladder(ldx)   # 持续按住朝梯走(正常大步移动,不是点动小碎步)
            return False
        ...
        if self._climb_state == "climbing":
            ...
            # === 到顶两阶段判定(用户2026-09-11晚定稿;背景静止法从爬梯摘除,_pick_climb_boxes/_climb_boxes_still函数保留给别处用) ===
            # 阶段1【主窗口】台子不平、不要求Y重合:人物基点Y 对 "抓住时冻结的记录坐标点Y",差≤CLIMB_TARGET_Y_TOL(20)且真爬过=到记录层高度
            #   → 补按住方向键 CLIMB_TOP_HOLD_MS(300) 把人翻上不平的台面(补按期间本段开头一直按住,不松);
            # 阶段2【小地图】补按满后,小地图光点Y 对锁梯连接端(上行梯顶/下行梯底)重合或单向越过(容差LADDER_TOP_ARRIVE_TOL)=最终到顶,当帧松键开主线。
            # 真爬/假抓住只认小地图光点朝爬向的真位移(不再每帧截图对比背景,顺带省CPU);补按满但小地图仍没到=台面更高,退补按继续爬,不死等。
            _end_y = self._climb_ladder_y_top if self._climb_direction > 0 else self._climb_ladder_y_bottom
            _real_dy = ((self._climb_start_y - py) if self._climb_direction > 0 else (py - self._climb_start_y)) \
                if self._climb_start_y else 0
            # 第一步"确认在爬"(只认小地图Y真位移+时间兜底):真移动锁存不可逆,纯时间兜底不算真动(假抓住也会到点)
            _y_climbing = _real_dy >= 3
            _confirm_to = bool(self._climb_action_time) and (now_ms - self._climb_action_time >= CLIMB_CONFIRM_FALLBACK_MS)
            if _real_dy >= CLIMB_REAL_MOVE_PX:
                self._climb_ever_moved = True
            if _y_climbing or _confirm_to:
                if not self._climb_move_confirmed:
                    self._climb_move_confirmed = True
                    _why_c = "人物Y移动" if _y_climbing else "%.0fms兜底" % CLIMB_CONFIRM_FALLBACK_MS
                    _debug_log("[爬梯] 第一步通过(%s)=确认在爬(光点Y=%.0f)" % (_why_c, py))
            # 小地图光点对梯端:重合/单向越过
            _map_ok = False
            if _end_y:
                _map_ok = (py <= _end_y + LADDER_TOP_ARRIVE_TOL) if self._climb_direction > 0 \
                    else (py >= _end_y - LADDER_TOP_ARRIVE_TOL)
            # 阶段1:主窗口Y差≤20(记录Y缺失时,用"小地图已到梯端"等价触发兜底,不死等)
            _sp_v = self._player_screen_pos
            _rec_y = getattr(self, '_climb_record_scr_y', None)
            _near = (_real_dy >= CLIMB_REAL_MOVE_PX and _sp_v and _rec_y is not None
                     and abs(_sp_v[1] - _rec_y) <= CLIMB_TARGET_Y_TOL) or bool(_map_ok)
            if _near and not self._climb_top_hold:
                self._climb_top_hold = True
                self._climb_top_hold_t = now_ms
                _debug_log("[爬梯] 主窗口Y差≤%d到记录高度(小地图到端=%s),补按方向键%dms再核对梯端(光点Y=%.0f)"
                           % (CLIMB_TARGET_Y_TOL, _map_ok, CLIMB_TOP_HOLD_MS, py))
            _arrived = False
            _arrive_why = ""
            if self._climb_top_hold and (now_ms - self._climb_top_hold_t) >= CLIMB_TOP_HOLD_MS and _map_ok:
                # 补按满300ms 且 小地图光点重合/越过梯端(容差1px)=到顶。补按满但还没重合:保持按住继续爬、
                # 【不反复退补按】(用户2026-09-11晚:小地图只作最终确认,不搞补按几次的死循环;方向键本段开头一直按住,12s总超时兜底)
                _arrived = True
                _arrive_why = "补按%dms+小地图光点到梯端" % CLIMB_TOP_HOLD_MS
            # 假抓住失败(用户2026-09-10):抓住满CLIMB_FAKE_GRAB_MS但小地图Y从没真移动(ever_moved=False)=根本没爬上梯,
            # 松键回主线,绝不判到顶/不触发"到达新平台清锁"(旧bug:人原地没动却一遍遍"到第二层清锁")
            if not _arrived and not self._climb_ever_moved and self._climb_action_time \
                    and now_ms - self._climb_action_time >= CLIMB_FAKE_GRAB_MS:
                _debug_log("[爬梯] 假抓住:起%.0fms小地图Y没真移动,判抓梯失败回主线(不判到顶)" % (now_ms - self._climb_action_time))
                self._rlog("抓梯%dms没真爬上(假抓住),回主线打怪" % CLIMB_FAKE_GRAB_MS, LOG_RED, log='behavior')
                ...
                self._decide_climb_fail_action()
                return False
            # 爬梯总超时(防异常永久卡的安全网,不是到顶判据),到点按到顶收尾
            if not _arrived and self._climb_action_time and now_ms - self._climb_action_time > CLIMB_TOTAL_TIMEOUT_MS:
                _arrived = True
                _arrive_why = "总超时%dms兜底收尾" % CLIMB_TOTAL_TIMEOUT_MS
            if _arrived:
                # 到顶当帧松上下方向键出梯、清锁梯开主线开打
                _debug_log("[爬梯] %s(光点Y=%.0f),松键开主线" % (_arrive_why, py))
                ...
                if VK_DOWN in self._random_move_keys:
                    self._key_up(VK_DOWN)
                self._reset_climb()
                # 梯子真正到顶出梯:清掉梯子上测的旧怪表+寻怪范围重扫重锁(防拿梯子上的下方旧怪判cross,一上去就下来)
            ...
            def on_mouse(e, x, y, fl, p):
                ...
                if ph == "place":
                    ...
                    if e == cv2.EVENT_LBUTTONDOWN:
                        ...
                        for k, q in enumerate(cur["pts"]):
                            d = abs(q[0]-oxn)+abs(q[1]-oyn)
                            if d < bd: bd, best = d, k
                        anc = cur["anchor"]
                        if anc is not None and abs(anc[0]-oxn)+abs(anc[1]-oyn) < bd:
                            cur["anchor"] = None
    ...
    def _ladder_align_by_screen(self, tpl_x, px, py, now_ms, jump_key):
        ...
        _rj_trig = (not getattr(self, '_ladder_run_jumped', False)) \
            and (getattr(self, '_lad_scr_running', False) or _moving_to_lad) \
            and LADDER_SCR_RUNJUMP_TRIG_LO <= asdx <= LADDER_SCR_RUNJUMP_TRIG_HI \
            and asdx <= LADDER_SCR_RUNJUMP_IDEAL + max(_approach_v, 0.0) \
            and _merged   # 已选中一把梯子白框才允许跑跳,没选中落下面趋近段继续靠近
        if _rj_trig:
            if _ak is not None:
                self._key_up(_ak)
                self._lad_scr_key_vk = None
            ...
            self._lad_scr_running = False   # 助跑态消费掉,防止连续重复跑跳
            self._ladder_vert_jumped = False
            self._ladder_run_dir = 1 if sdx > 0 else -1
            self._ladder_run_t0 = now_ms
            self._climb_start_y = py
            self._press_game_key(jump_key, duration=80)
            _debug_log("[爬梯·屏幕·跑跳] 梯X=%d 人X=%d 差%.1f(上帧%s 周期趋近v=%.1f)自适应提前起跳→移动中跑跳(朝%s不松)" % (
                tpl_x, spx, sdx, ("%.1f" % _prev_asdx) if _prev_asdx is not None else "NA",
                _approach_v, "右" if sdx > 0 else "左"))
            self._rlog("跑跳上梯(X差%.1f,移动中起跳,跳完80ms松左右只按↑)" % sdx, log='behavior')
            self._ladder_jump_phase = 'post_jump'
            self._ladder_post_jump_step = 'run_delay'
            self._ladder_post_jump_t = now_ms
            return False
        # 段2.5:35<X差≤100却没被段2跑跳消费(没合并/速度判据没到/跑跳机会已用掉)→按住朝梯单调正常走:
        # 没跑跳过就置回助跑态(还在70~100下帧段2补跑跳);已跑跳失败降级或已跌破70就一路按住走到≤35进段3碎步、≤5直跳。不reset、不转打怪、不左右摆。
        if LADDER_SCR_BO_LO < asdx <= LADDER_SCR_BO_HI:
            if _ak is not None:
                self._key_up(_ak)
                self._lad_scr_key_vk = None
    ...
    def _lock_target_ladder_for_cross(self, mpx, mpy, sx, sy):
        ...
        (怪是空怪/下帧脱检/刷新移位都不影响)。返回锁定梯dict;换算不出/无Y合格梯→None(本帧不启动,下帧用新怪再选)。
        注意:人当前够不着时_find可能返回None但梯已锁(需走近/中转),故返回值以self._locked_ladder为准。"""
        if getattr(self, '_locked_ladder', None) is not None:
            return self._locked_ladder
        try:
            _mp = self._get_monster_map_pos_verified(sx, sy)
        except Exception:
            _mp = None
        if _mp is None:
            return None
        self._find_nearest_ladder(mpx, mpy, float(_mp[1]), monster_x=float(_mp[0]))
        return getattr(self, '_locked_ladder', None)
    ...
    def _try_platform_transition(self, cross_candidates, now):
        ...
        # 屏幕怪只用于"绿线相连走walk"判断和"选梯那一下的怪→梯→人";锁梯后引路只认锁定梯,
        # 怪空/脱检/刷新(cross_candidates为空)都不影响继续朝锁梯走,绝不能因此站桩(用户2026-09-11)。
        fx = fy = None
        if cross_candidates:
            # 有怪：取最近的怪
        ...
        if fx is not None:
            ...
            if monster_pf and cur_pf and inter:
                ...
                # 走梯子(怪不在绿线/绿线不相连):选梯那一下由当前怪【怪→梯→人】锁定正确梯;锁过后引路只认锁梯、不再依赖怪
                if monster_pf:
                    _debug_log("[跨层] 怪在绿线但人物不在绿线或绿线不相连，改走梯子")
                if getattr(self, '_locked_ladder', None) is None:
                    self._lock_target_ladder_for_cross(mpx, mpy, fx, fy)
        # 【走梯引路终点=锁定梯连接端·用户2026-09-11】上行取梯顶端y_top/下行取底端y_bottom,到端=到层=开主线;
        # 已锁梯就算本帧没cross怪(fx is None,怪空/脱检/刷新)也照样朝锁梯走,不被打断;没锁成才退回本帧怪再锁一次。
        if target_mid is None and self._transit_via != 'walk':
            _lad = getattr(self, '_locked_ladder', None)
            if _lad is None and fx is not None:
                _lad = self._lock_target_ladder_for_cross(mpx, mpy, fx, fy)
            if _lad is not None:
                _up = float(_lad["y_top"]) < mpy
                _end_y = float(_lad["y_top"]) if _up else float(_lad["y_bottom"])
                target_mid = (float(_lad["x"]), _end_y)
        if target_mid is None and fx is None:
            # 没锁梯、本帧也没cross怪:选台模式→去下一个选中台子;全图模式(未选台)→原地等刷怪。
            if self._selected_platforms:
                cur_pf = self._get_current_manual_platform()
                ...
                self._trans_stall_diag('idle_no_platform(无锁定梯且全图无怪未选台,原地等刷)', now)
                return False  # 全图模式无怪、也没锁梯：原地等刷怪
        ...
        if _near_map and cross_candidates and self._player_screen_pos:
            try:
                _fc_an = self._get_fight_config()
                _atk_up = abs(int(_fc_an.get("attack_y_up", -80) or -80))
                _atk_dn = abs(int(_fc_an.get("attack_y_down", 80) or 80))
                _atk_dist = int(_fc_an.get("atk1_distance", 150) or 150)
            except (TypeError, ValueError):
                _atk_up, _atk_dn, _atk_dist = 80, 80, 150
            _c0 = sorted(cross_candidates)[0]          # 最近cross怪(屏幕坐标)=(dist,screen_x,screen_y)
            _screen_x_gap = abs(_c0[1] - self._player_screen_pos[0])
            _screen_layer_gap = abs(_c0[2] - self._player_screen_pos[1])
            # 【2026-09-09修复下行被already_near误杀·真机"目标在下方37px/X差151却呆站不跳"】必须屏幕上X也进射程、
            # Y也同层带内才算"真在身边";scale把X差压扁会让小地图_near_map成立、屏幕X却还差很远→放行跨层(下行下跳/走梯),不许原地already_near
            _same_layer_screen = (_screen_layer_gap <= max(40, _atk_up, _atk_dn)
                                  and _screen_x_gap <= _atk_dist)
        if _near_map and (not cross_candidates or _same_layer_screen):
            self._trans_stall_diag('already_near(目标换算后就在身边)', now,
                                   ...
                                   screen_xgap=_screen_x_gap, screen_ygap=_screen_layer_gap)
            return False
        if _near_map:
            _debug_log("[跨层] 小地图看似已近但屏幕仍分层/偏远(X差%d Y差%d),继续跨层不走already_near(治scale压扁)" % (_screen_x_gap, _screen_layer_gap))
        ...
        self._transit_walk_from = self._player_map_pos  # 走台启动时人物坐标(到点判真移动,防空到达小碎步)
        self._arrival_empty_streak = 0
        self._transit_walk_last_x = None
        self._transit_walk_stall = 0
    ...
    def _player_loop(self):
        ...
        while self._detect_running:
            ...
            _precise_now = bool(getattr(self, '_ladder_precise_mode', False)) \
                and getattr(self, '_climb_state', 'none') in ('to_ladder', 'climbing')
            if getattr(self, '_ladder_precise_mode', False) and not _precise_now:
                self._ladder_precise_mode = False   # 保险:已离开上梯流程却残留高频标志,自动关恢复正常全检测
    ...
    def _combat_tick(self):
        ...
        if self._combat_transit or getattr(self, '_climb_state', 'none') == 'to_ladder':
            ...
            if self._ladder_backoff is not None and self._ladder_backoff_step(now):
                return
            if _dl['state'] == 'cast' and _dl.get('target'):
                # 平地去梯途中近身刷出能直打的怪:三步走解绑回主线(用户2026-09-11,顺序不可乱)
                self._abort_transit_for_nearby()
                return
            self._transit_step()
            return
                ...
                try:
                    ...
                    if getattr(self, '_climb_state', 'none') != 'none' and self._player_screen_pos:
                        try:
                            ...
                            _choose = None   # (排序键, 白框中心X, 白框中心Y)
                            _pick_src = None
                            _tri_rows = []
                            # 主方案【三点折线 怪→梯→人】:目标怪在本屏可见→X总程最短的白框,纯几何、不依赖会漂的倍率
                            if _tri_on:
                                _tri_rows = self._pick_ladder_by_triangle(
                                    int(_monp[0]), int(_monp[1]), _white_marks, _psx, _psy, _cdir)
                                if _tri_rows:
                                    # 滞回:上帧选中梯没跑出保持带就继续钉它,不在两把近邻梯间横跳
                                    _hold = [r for r in _tri_rows
                                             if _prev is not None and abs(r[1] - _prev) <= LADDER_MARK_HOLD_X]
                                    _row = _hold[0] if _hold else _tri_rows[0]
                                    _choose, _pick_src = (_row[0], _row[1], _row[2]), '三点'
                            # 兜底:怪不在屏/三点无候选→纯白框就近:按上下方向滤掉反方向梯后,选离人物基点最近的一把(滞回钉住)
                            if _choose is None:
                                _cand = []
                                for (_wx, _wy) in _white_marks:
                                    if _cdir > 0 and _wy > _psy + 40:
                                        continue   # 上行却在脚下=另一把下行梯,排除
                                    if _cdir < 0 and _wy < _psy - 40:
                                        continue   # 下行却在头顶=另一把上行梯,排除
                                    _cand.append((abs(_wx - _psx), _wx, _wy))
                                _mc = None
                                if _prev is not None:
                                    _h = [c for c in _cand if abs(c[1] - _prev) <= LADDER_MARK_HOLD_X]
                                    if _h:
                                        _mc = min(_h, key=lambda c: c[0])
                                if _mc is None and _cand:
                                    _mc = min(_cand, key=lambda c: c[0])
                                if _mc is not None:
                                    _choose, _pick_src = _mc, '就近'
                            _snap = _choose is not None
                            if _snap:
                                _rx, _ry = _choose[1], _choose[2]
                                self._ladder_snap_x = _rx   # 选中白框的真实中心X,供对齐起跳(人物基点对到白框中心)
                                self._lad_snap_prev = _rx
                                _sel = (_rx, _ry, True)
                            else:
                                self._ladder_snap_x = None
                            # 观测日志(节流300ms):三点按 怪→梯→人 打每把梯总程;就近打离人距离
                            if _now_lm - getattr(self, '_snap_dbg_t', 0) >= 300:
                                self._snap_dbg_t = _now_lm
                                if _tri_on:
                                    _detail = ' '.join('梯%d(怪→梯%d+梯→人%d=总%d%s)' % (
                                        r[1], r[4], r[5], r[0], '间' if r[3] else '')
                                        for r in _tri_rows[:4]) or '无同向候选'
                                    _debug_log("[三点选梯·怪→梯→人] 怪X=%d 人X=%d 向%s|%s →来源%s 选中白框X=%s" % (
                                        int(_monp[0]), _psx, ('上' if _cdir > 0 else '下'), _detail,
                                        _pick_src or '无', _rx if _snap else None))
                                else:
                                    _allx = ','.join(str(wx) for wx, wy in _white_marks) or '无'
                                    _debug_log("[白框就近选梯] 人X=%d 向%s|白框[%s]→%s 选中X=%s" % (
                                        _psx, ('上' if _cdir > 0 else '下'), _allx,
                                        _pick_src or '未选中(没扫到白框)', _rx if _snap else None))
                        except Exception:
                            _sel = None
