...
class MinimapRouteRecorder:
    ...
    def _combat_tick(self):
        ...
        if now < self._combat_idle_until:
            return
        if now - self._combat_last_idle_check > random.randint(10000, 20000):
            self._combat_last_idle_check = now
            if random.random() < 0.3:
                self._combat_idle_until = now + random.randint(1000, 3000)
        ...
        if not self._combat_had_target:
            ...
            self._combat_react_until = now + random.randint(150, 500)
            return
        ...
        if now - self._combat_last_move > random.randint(2000, 5000):
            ...
            if random.random() < 0.2:
                ...
                self._combat_busy_until = now + random.randint(100, 250)
                return
        ...
        if now - self._combat_last_jump > random.randint(5000, 12000):
            ...
            if random.random() < 0.25:
                self._press_game_key("alt", duration=random.randint(50, 100))
                self._combat_busy_until = now + random.randint(200, 400)
                return
        ...
        if not skill_cast and aoe_key:
            ...
            if in_range >= 3 and now - last > aoe_cd + random.randint(-skill_rand, skill_rand):
                if random.random() < 0.8:
                    ...
                    skill_cast = True
                    self._combat_busy_until = now + random.randint(300, 800)
        ...
        if not skill_cast and atk_key:
            ...
            if t_dist <= atk_dist and now - last > atk_cd + random.randint(-skill_rand, skill_rand):
                ...
                skill_cast = True
                self._combat_busy_until = now + random.randint(200, 600)
