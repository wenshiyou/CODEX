...
class MinimapRouteRecorder:
    ...
    def _is_mp_label_visible(self, frame):
        """第一次全窗口搜索定位MP标签坐标并永久缓存，后续只在固定坐标±15px范围内搜。
        搜索范围=蒙板画框范围，超出窗口自动缩小。搜不到=被挡，不回退全窗口。"""
        if self._mp_label_template is None or frame is None:
            return True
        th, tw = self._mp_label_template.shape[:2]
        h, w = frame.shape[:2]
        pad = 15  # 固定坐标周围的搜索/画框半径

        cached = getattr(self, '_mp_label_cached_pos', None)
        if cached is None:
            # 第一次：全窗口搜索定位，永久缓存坐标
            result = cv2.matchTemplate(frame, self._mp_label_template, cv2.TM_CCOEFF_NORMED)
            _, max_val, _, max_loc = cv2.minMaxLoc(result)
            abs_x, abs_y = max_loc
            self._mp_label_cached_pos = (abs_x, abs_y)
            print("[MP标签] 首次定位: 匹配度=%.3f 坐标=(%d,%d) 已固定" % (max_val, abs_x, abs_y))
        else:
            # 后续：固定坐标±pad小范围搜，不回退
            cx, cy = cached
            x1 = max(0, cx - pad)
        ...
        # 搜索范围=画框范围：以固定缓存坐标为中心±pad，超出窗口自动缩小
        cx, cy = self._mp_label_cached_pos
        bx1 = max(0, cx - pad)
        by1 = max(0, cy - pad)
        bx2 = min(w, cx + tw + pad)
        by2 = min(h, cy + th + pad)
        self._mp_label_search_rect = (bx1, by1, bx2 - bx1, by2 - by1)
        self._mp_label_pos = (abs_x, abs_y, tw, th, visible)
        if not visible:
            print("[MP标签] 固定范围内未找到(匹配度=%.3f)，判定被挡" % max_val)
        return visible
    ...
    def _check_auto_potion(self):
        ...
        if self._monster_overlay_running:
            ...
            # MP标签搜索范围（搜索范围=画框范围，带visible标记）
            if hasattr(self, '_mp_label_search_rect') and self._mp_label_search_rect:
                sx, sy, sw, sh = self._mp_label_search_rect
                vis = getattr(self, '_mp_label_pos', (0,0,0,0,False))[4] if hasattr(self, '_mp_label_pos') else True
                self._monster_overlay_data['mp_label_search'] = (sx, sy, sw, sh, vis)
    ...
    def _filter_monsters_on_platform(self, monsters, player_screen_pos):
        ...
        for m in monsters:
            x1, y1, x2, y2, score = m
            if abs(y2 - py) <= 35:  # 怪脚 vs 人手，缩小范围避免跨层
    ...
    def _is_monster_on_platform(self, monster_cx, monster_cy):
        ...
        if not current_pf or not self._player_map_pos or not self._player_screen_pos:
            if self._player_screen_pos:
                return abs(monster_cy - self._player_screen_pos[1]) <= 35
            return False
