...
class MinimapRouteRecorder:
    ...
    def _start_auto_calibration(self, axis='X'):
        ...
        # 完成阶段(stage3→0)：算倍率、清空、退出
        if self._auto_calib_stage == 3:
            self._finish_auto_calibration()  # 按axis只算对应轴
            self._reset_calib_state()  # 清空校准点位
            self._auto_calib_stage = 0  # 回到空闲
            return
        ...
        # 第3步：人物走到光圈，记录人物小地图位置（取对应轴分量）
        if self._auto_calib_stage == 2:
            if axis == 'X':
                self._auto_calib_green_map = (cur_mx, cur_my)  # 只取X点小地图坐标
            else:
                self._auto_calib_blue_map = (cur_mx, cur_my)   # 只取Y点小地图坐标
    ...
    def _on_mouse(self, event, x, y, flags, param):
        ...
        # 【台子选择按钮点击】直接检测，不放在小地图区域内，避免小地图尺寸变化导致点击不到
        if event == cv2.EVENT_LBUTTONDOWN:
            _in_result = _in(self._btn_platform_selector, x, y) if self._btn_platform_selector else "btn_is_None"
            debug_msg = "[调试台子2] 点击=(%d,%d) btn=%s event_is_LDOWN=%s _in_result=%s" % (
                x, y, self._btn_platform_selector, (event == cv2.EVENT_LBUTTONDOWN), _in_result)
            print(debug_msg)
            # 保存到日志文件，方便自己查看
            try:
                with open("platform_selector_debug2.log", "a", encoding="utf-8") as f:
                    f.write(debug_msg + "\n")
            except Exception:
                pass
            # 点击检测代码放在同一个if语句内部，避免缩进问题
            if self._btn_platform_selector and _in(self._btn_platform_selector, x, y):
                debug_msg3 = "[调试台子3] 进入点击检测分支，准备切换_show_platform_selector，当前值=%s" % self._show_platform_selector
                print(debug_msg3)
                try:
                    with open("platform_selector_debug2.log", "a", encoding="utf-8") as f:
                        f.write(debug_msg3 + "\n")
                ...
                debug_msg4 = "[台子选择] 打开面板" if self._show_platform_selector else "[台子选择] 关闭面板"
                print(debug_msg4)
                try:
                    with open("platform_selector_debug2.log", "a", encoding="utf-8") as f:
                        f.write(debug_msg4 + "\n")
                ...
                debug_msg3 = "[调试台子3] 未进入点击检测分支，btn_selector=%s _in=%s" % (
                    bool(self._btn_platform_selector), _in_result)
                print(debug_msg3)
                try:
                    with open("platform_selector_debug2.log", "a", encoding="utf-8") as f:
                        f.write(debug_msg3 + "\n")
                    ...
                    pass
        # 【台子选择面板中的点击】直接检测，不放在小地图区域内
        if event == cv2.EVENT_LBUTTONDOWN and self._show_platform_selector and self.platforms:
            # 面板位置：基于台子选择按钮的位置动态计算，和绘制位置保持一致
            panel_x = self._btn_platform_selector[0] - 5 + 10
            panel_y = self._btn_platform_selector[1] + self._btn_platform_selector[3] + 5
            panel_w = UI_MAP_W - 20
            # 关闭按钮X
    ...
    def draw(self, map_area, player_pos):
        ...
        if self._show_platform_selector and self.platforms:
            # 面板位置：基于台子选择按钮的位置动态计算，和点击检测位置保持一致
            panel_x = btn_sel_x - 5 + 10  # 和按钮左对齐，稍微向右一点
            panel_y = btn_sel_y + btn_sel_h + 5  # 在按钮下方
