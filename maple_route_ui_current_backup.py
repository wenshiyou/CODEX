...
# 【模块B】自动校准按钮（同屏三点校准：基点+右800+上500，按效果图调整位置大小）
...
class MinimapRouteRecorder:
    def __init__(self):
        ...
        self._calib_overlay_msg = None  # 蒙板正中间临时提示文字（如截图失败提示），(文字, 颜色BGR, 截止时间戳)
        ...
        self._calib_template_size = 54       # 模板截图大小（54x54像素，half=27，27*2=54）
        self._calib_match_threshold = 0.78  # 模板匹配置信度阈值（0.78兼顾准确率和召回率）
        # 热键跑马灯滚动偏移（从右到左流动）
        self._hotkey_scroll_x = 0
        # 热键跑马灯预加载字体和文字尺寸（避免每帧加载和计算导致卡顿）
        self._hotkey_text = "小提示：F5平台录制，F6梯子录制，F7方案清除，F8方案保存，F9手动截取小地图，F10开始运行，F12停止运行"
        try:
            self._hotkey_font = ImageFont.truetype("simhei.ttf", 24)
            # 预计算文字宽度高度，避免每帧textbbox计算卡顿
    ...
    def _detect_minimap(self, debug=True):
        ...
        # 2. 找"大地图"文字（小地图右侧同行）
        roi_b_x1 = mini_x + mw
        roi_b_x2 = min(fw, mini_x + 400)  # X范围加大到400，避免右边识别不到
        ...
        if roi_btm.shape[0] >= btm_h and roi_btm.shape[1] >= btm_w:
            ...
            if val_btm >= 0.55:
                # 底部边界定在模板顶部再向上移15像素（去掉灰色边框区域）
                bottom = search_y1 + loc_btm[1]  # 底部边界定在模板顶部（向下移15px回原位）
                if debug:
        ...
        TITLE_PAD = 53  # 从上边界向下移53px开始截取（原45，向下移8px）
        new_map = {
            "left": left,
            "top": top + TITLE_PAD,
            "width": right - left,
        ...
        if os.path.exists(calib_file):
            try:
                ...
                if saved_sx and saved_sy:
                    ...
                    print("[校准] 加载保存倍率: X=%.4f Y=%.4f" % (self._calibrated_scale_x, self._calibrated_scale_y))
                # 加载校准数据后重新计算倍率，确保小地图底部状态栏显示倍率
                    ...
                    if dx_screen > 0 and dx_map > 0 and dy_screen > 0 and dy_map > 0:
                        ...
                        self._map_screen_scale = self._calibrated_scale_x
                        print("[校准] 加载倍率: X=%.4f Y=%.4f" % (self._calibrated_scale_x, self._calibrated_scale_y))
            except Exception:
                pass
        self._save_route_config()
    ...
    def find_player_dot(self, map_area):
        """【模块B】检测小地图上人物黄色光点（带跟踪逻辑，避免误检测远处黄色物体）
        原理：HSV颜色过滤找黄色区域，有上次位置时取离上次最近的轮廓（跟踪），否则取面积最大的
        返回：(x, y) 光点正中心坐标；找不到返回None"""
        ...
        if valid:
            ...
            if centers:
                ...
                if last_pos:
                    # 有上次位置：取离上次最近的轮廓（跟踪逻辑，避免误检测远处黄色物体）
                    best = min(centers, key=lambda p: (p[0]-last_pos[0])**2 + (p[1]-last_pos[1])**2)
                    # 最大距离限制：离上次位置超过60像素说明跟踪丢了，重新取面积最大的（避免一直跟踪错误位置）
    ...
    def _start_auto_calibration(self):
        ...
        # 【调试0-入口】方法被调用，输出当前stage和人物位置状态
        self._rlog("【调试0】F11被调用 stage=%d, 屏幕位置=%s, 小地图位置=%s" % (
        ...
        if self._auto_calib_stage == 4:
            ...
            return
        # 【关键】直接用主循环实时更新的_player_map_pos，光点在哪就记录在哪，不重新检测避免坐标不同步
        # 调试：记录当前_player_map_pos值到文件
        try:
            with open(os.path.join(DATA_DIR, "calib_debug.log"), "a", encoding="utf-8") as f:
                f.write("[%s] 按F11前 stage=%d _player_map_pos=%s _player_screen_pos=%s\n" % (
        ...
        if self._auto_calib_stage in (0, 1):
            ...
            if not self._player_map_pos:
                # 第三步失败提示绿光圈，第四步失败提示蓝光圈
                if self._auto_calib_stage == 2:
                    self._add_log("绿光圈位置记录失败 请重按 F11", (0, 0, 255))
                else:
                    self._add_log("蓝光圈位置记录失败 请重按 F11", (0, 0, 255))
                # 失败三次后直接结束整个过程
                self._calib_retry_count = getattr(self, '_calib_retry_count', 0) + 1
                if self._calib_retry_count >= 3:
                    self._add_log("连续3次失败，校准过程已关闭", (0, 0, 255))
                    self._auto_calib_stage = 0
                    self._auto_calib_base = None
                    self._calib_green_template = None
                    self._calib_blue_template = None
        ...
        if self._auto_calib_stage == 0:
            ...
            try:
                with open(os.path.join(DATA_DIR, "calib_debug.log"), "a", encoding="utf-8") as f:
                    f.write("[%s] stage0→1 开始校准 人物小地图=(%d,%d) 屏幕=(%d,%d)\n" % (
        ...
        if self._auto_calib_stage == 1:
            # 【调试1-业务层】进入阶段1→2，输出当前状态
            self._rlog("【调试1】进入第二步 stage=1, 人物屏幕=(%d,%d) 小地图=(%d,%d)" % (cur_sx, cur_sy, cur_mx, cur_my))
            # 显示提示：请等待自动截图保存
            self._add_log("请等待自动截图保存...", (255, 165, 0))
            ...
            # 【调试2-坐标层】输出绿点蓝点屏幕坐标和偏移
            self._rlog("【调试2】绿点屏幕=(%d,%d) 偏移=(%d,%d), 蓝点屏幕=(%d,%d) 偏移=(%d,%d)" % (
            ...
            for _green_try in range(3):
                ...
                if _g_ret and self._calib_green_template is not None:
                    ...
                    self._rlog("【调试4】绿点截图第%d次: 返回=%s 模板尺寸=%dx%d" % (_green_try + 1, _g_ret, _gw, _gh))
                    if _gh == self._calib_template_size and _gw == self._calib_template_size:
                        _green_ok = True
                        self._add_log("绿色光圈模板保存成功 %dx%d" % (_gw, _gh), (0, 200, 0))
                        break
                else:
                    self._rlog("【调试4】绿点截图第%d次: 返回=%s 模板=None" % (_green_try + 1, _g_ret))
                if _green_try < 2:
                    time.sleep(0.3)
            if not _green_ok:
                # 截图失败后先恢复蒙板显示，避免蒙板一直隐藏用户以为卡住
                if self._overlay_hwnd:
                    user32.ShowWindow(self._overlay_hwnd, 5)  # SW_SHOW
                self._rlog("【调试5】绿点截图失败，恢复蒙板，stage保持1")
                self._add_log("截图保存失败 请重新按 F11", (0, 0, 255))
                # 失败三次后直接关闭整个过程，不再显示基点，不回第一步
                self._calib_retry_count = getattr(self, '_calib_retry_count', 0) + 1
                if self._calib_retry_count >= 3:
                    self._add_log("连续3次失败，校准过程已关闭", (0, 0, 255))
                    self._auto_calib_stage = 0
            ...
            # 【调试6-识别层】开始截图蓝点模板
            self._rlog("【调试6】绿点截图成功，等待1秒后开始截图蓝点模板")
            # 截图蓝点位置的背景模板
            _blue_ok = False
            for _blue_try in range(3):
                ...
                if _b_ret and self._calib_blue_template is not None:
                    ...
                    self._rlog("【调试7】蓝点截图第%d次: 返回=%s 模板尺寸=%dx%d" % (_blue_try + 1, _b_ret, _bw, _bh))
                    if _bh == self._calib_template_size and _bw == self._calib_template_size:
                        _blue_ok = True
                        self._add_log("蓝色光圈模板保存成功 %dx%d" % (_bw, _bh), (0, 200, 0))
                        break
                else:
                    self._rlog("【调试7】蓝点截图第%d次: 返回=%s 模板=None" % (_blue_try + 1, _b_ret))
                if _blue_try < 2:
                    time.sleep(0.3)
            if not _blue_ok:
                # 截图失败后先恢复蒙板显示，避免蒙板一直隐藏用户以为卡住
                if self._overlay_hwnd:
                    user32.ShowWindow(self._overlay_hwnd, 5)  # SW_SHOW
                self._rlog("【调试8】蓝点截图失败，恢复蒙板，stage保持1")
                self._add_log("截图保存失败 请重新按 F11", (0, 0, 255))
                # 失败三次后直接关闭整个过程，不再显示基点，不回第一步
                self._calib_retry_count = getattr(self, '_calib_retry_count', 0) + 1
                if self._calib_retry_count >= 3:
            ...
            # 恢复蒙板显示
            # 【调试9-业务层】截图全部成功，先设stage=2再恢复蒙板，避免箭头闪烁
            self._rlog("【调试9】绿点蓝点截图全部成功，stage从1变成2，再恢复蒙板显示")
            self._auto_calib_stage = 2
            # 先设stage=2再恢复蒙板，这样恢复蒙板时直接显示绿圈蓝圈，不会显示箭头闪烁
            if self._overlay_hwnd:
                user32.ShowWindow(self._overlay_hwnd, 5)  # SW_SHOW
            ...
            try:
                with open(os.path.join(DATA_DIR, "calib_debug.log"), "a", encoding="utf-8") as f:
                    f.write("[%s] stage1→2 固定基点 屏幕=(%d,%d) 小地图=(%d,%d)\n" % (
        ...
        if self._auto_calib_stage == 3:
            ...
            try:
                with open(os.path.join(DATA_DIR, "calib_debug.log"), "a", encoding="utf-8") as f:
                    f.write("[%s] stage3→4 记录蓝点 小地图=(%d,%d)\n" % (time.strftime("%H:%M:%S"), cur_mx, cur_my))
            except Exception:
                pass
            return
    ...
    def _capture_calib_template(self, screen_pos, color_tag):
        ...
        返回：True成功，False失败
        截图同时保存到calib_screenshots文件夹，方便用户确认截图是否正确"""
        try:
            frame = self._capture_window()
            if frame is None:
                return False
            ...
            # 确保模板完整55x55，靠近边缘自动内移（避免边界截断导致匹配失败）
            ...
            _debug_log("[校准模板] %s截图成功 %dx%d at (%d,%d), 已保存到 %s" % (color_tag, template.shape[1], template.shape[0], sx, sy, save_path))
            return True
        except Exception as e:
    ...
    def _match_calib_templates(self):
        ...
        添加调试信息：输出匹配值和位置，保存调试图，方便排查识别不到的问题"""
        if self._auto_calib_stage < 2:
            return
        ...
        try:
            ...
            if frame is None:
                return
            debug_frame = frame.copy()  # 调试图副本
            # 奇偶帧交替匹配：奇数帧匹配绿点，偶数帧匹配蓝点，一次只匹配一个，避免互相干扰
            if self.frame_count % 2 == 1:
                ...
                if self._calib_green_template is not None:
                    ...
                    _debug_log("[校准匹配] 绿点匹配值: %.4f, 阈值: %.2f, 位置: (%d,%d)" % (
                        max_val, self._calib_match_threshold, max_loc[0], max_loc[1]))
                    # 输出匹配值到UI日志，方便排查
                    if self.frame_count % 30 == 0:
                        self._rlog("【匹配】绿点值=%.3f 蓝点值=%.3f 阈值=%.2f" % (
                ...
                if self._calib_blue_template is not None:
                    ...
                    _debug_log("[校准匹配] 蓝点匹配值: %.4f, 阈值: %.2f, 位置: (%d,%d)" % (
                        max_val, self._calib_match_threshold, max_loc[0], max_loc[1]))
                    # 在调试图上画出蓝点匹配位置和匹配值
            ...
            _debug_log("[校准匹配] 调试图已保存到 %s" % debug_path)
        except Exception as e:
            _debug_log("[校准模板匹配] 异常: %s" % e)

    def _save_calib(self):
        """保存端点数据到文件（左/右/上端点 + 倍率值）"""
        try:
            calib_file = os.path.join(DATA_DIR, "route_%d_calib.json" % self.current_route)
    ...
    def _handle_hotkey(self, vk):
        if vk == VK_F5:
            ...
            # 【调试-热键层】F11按键被捕获，输出当前状态
            self._rlog("【调试-热键】F11被按下 stage=%d, 蒙板hwnd=%s, 蒙板可见=%s" % (
    ...
    def draw(self, map_area, player_pos):
        with open("draw_debug.txt", "a", encoding="utf-8") as _f:
            _f.write("【draw】入口 tab=%s platforms=%d ladders=%d\n" % (self._current_tab, len(self.platforms), len(self.ladders)))
        ...
        self._last_map_w = w
        self._last_map_h = h
        # 【模块B】在小地图上画自动校准点（基点红、绿点、蓝点都画，方便确认位置）
        ...
        CALIB_DOT_R = 3
        # 红点（基点）：stage>=1时显示，和人物黄色光点重合
        if auto_base and len(auto_base) >= 4 and auto_stage >= 1:
            bx, by = int(auto_base[2]), int(auto_base[3])  # 基点小地图坐标
            if 0 <= bx < w and 0 <= by < h:
                cv2.circle(display, (bx, by), CALIB_DOT_R, (0, 0, 255), -1)  # 红色实心圆
        ...
        if calib_sx > 0 and calib_sy > 0:
            ...
            # 红色字，放在小地图左下角，无背景条纹，字体0.5，线宽1不加粗
        ...
        _hk_h = 36                 # 红框高=36
        _hk_text = self._hotkey_text  # 复用预定义文字，避免每帧重复定义
        # 滚动偏移每帧递减4像素（从右到左流动，速度适中不卡顿），文字完全移出左边界后重置到右边缘
        ...
        try:
            ...
            _tw = self._hotkey_text_w  # 复用预计算文字宽度，避免每帧textbbox计算卡顿
            _th = self._hotkey_text_h  # 复用预计算文字高度
    ...
    def manual_select_region(self):
        ...
        cv2.namedWindow(sel_win, cv2.WINDOW_AUTOSIZE)  # 自动适应图像大小，避免标题栏边框导致显示区域不一致
    ...
    def _add_log(self, msg, color=None):
        """添加日志，color为可选颜色参数（BGR元组），用于区分日志类型"""
            ...
            try:
                ...
                # === 自动校准蒙板拖动（仅stage=1时，拖动绿点蓝点定特色位置）===
                    ...
                    try:
                        ...
                        # stage=2: 绿圈蓝圈都显示；stage=3: 绿点已记录，绿圈消失，只显示蓝圈；stage=4: 都消失
                        if auto_stage == 2 or auto_stage == 3:
                            # 绿色特色位置绿光圈（仅stage=2显示，stage=3绿点已记录后消失）
                            if auto_stage == 2:
                            ...
                            # 蓝色特色位置蓝光圈（stage=2和3都显示，stage=4蓝点已记录后消失）
        ...
        if not occluded:
            ...
            if hp_blank and cfg.get("hp_key"):
                if self._hp_pot_wait_until == 0:
                    self._hp_pot_wait_until = now + random.randint(0, 500)  # 触发后等待延时0-500ms
                if now >= self._hp_pot_wait_until and now - self._last_hp_pot > self._hp_pot_delay:
                    ...
                    self._hp_pot_delay = random.randint(800, 1000)  # 吃药后冷却延时800-1000ms
            ...
            if mp_blank and cfg.get("mp_key"):
                if self._mp_pot_wait_until == 0:
                    self._mp_pot_wait_until = now + random.randint(0, 500)  # 触发后等待延时0-500ms
                if now >= self._mp_pot_wait_until and now - self._last_mp_pot > self._mp_pot_delay:
                    ...
                    self._mp_pot_delay = random.randint(800, 1000)  # 吃药后冷却延时800-1000ms
    ...
    def run(self):
        ...
        while True:
            with open("loop_debug.txt", "a", encoding="utf-8") as _lf:
                _lf.write("【loop】frame=%d\n" % getattr(self, 'frame_count', -1))
            try:
            ...
            self.frame_count += 1
            # 【调试-心跳】每秒输出一次，确认主循环是否在运行
            if self.frame_count % 30 == 0:
                self._rlog("【心跳】主循环运行中 frame=%d stage=%d" % (self.frame_count, self._auto_calib_stage))
            ...
            try:
                ...
                import traceback
                with open("draw_error.txt", "a", encoding="utf-8") as _ef:
                    _ef.write("draw error: %s\n%s\n" % (str(e), traceback.format_exc()))
                print("draw error:", e)
