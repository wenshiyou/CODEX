...
class MinimapRouteRecorder:
    def __init__(self):
        ...
        self._calib_template_size = 55       # 模板截图大小（55x55像素，更大更有特色，匹配更稳定）
    ...
    def draw(self, map_area, player_pos):
        ...
        self._last_map_w = w
        self._last_map_h = h
        # 【模块B】在小地图上画自动校准点（基点不画用游戏自带黄点，绿点stage>=2显示，蓝点stage=3显示）
        ...
        # 【模块B】自动校准倍率显示（显示在小地图底部状态栏，替代随机状态文字）
        ...
        if calib_sx > 0 and calib_sy > 0:
            ...
            cv2.rectangle(map_display, (0, MAP_H - 20), (FIXED_W, MAP_H), (25, 25, 25), -1)
            cv2.putText(map_display, scale_text, (6, MAP_H - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 255), 1)
                    ...
                    try:
                        ...
                        if auto_stage >= 1:
                            ...
                            if step_txt:
                                ...
                                # 文字显示在蒙板正中间（固定偏移居中，避免GetTextExtent调用不稳定）
                                txt_x = max(10, rect.right // 2 - 250)
                                txt_y = max(10, rect.bottom // 2 - 40)
