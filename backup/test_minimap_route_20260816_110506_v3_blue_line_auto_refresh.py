...
class MinimapRouteRecorder:
    def __init__(self):
        ...
        # 热键状态（GetAsyncKeyState 轮询，不受UIPI限制）
        self._key_state = {vk: False for vk in [VK_F5, VK_F6, VK_F7, VK_F8, VK_F9]}
        print("Hotkeys: F5=platform F6=ladder F7=clear F8=save F9=manual")
    ...
    def _on_mouse(self, event, x, y, flags, param):
        ...
        if idx == 0:
            ...
            print("[鼠标] 清除")
            self._handle_hotkey(VK_F7)
        elif idx == 3:
            print("[鼠标] 保存")
            self._handle_hotkey(VK_F8)
        elif idx == 4:
            print("[鼠标] 手动")
            self._handle_hotkey(VK_F9)
        elif idx == 5:
            print("[鼠标] 刷新-重新检测")
            self._detect_minimap()
            self.frame_count = 0
    ...
    def draw(self, map_area, player_pos):
        ...
        # 底部按钮栏
        btn_bar = np.zeros((BTN_H, FIXED_W, 3), dtype=np.uint8)
        btn_bar[:] = (45, 45, 45)
        btn_w = FIXED_W // 6
        buttons = [("平台", (0, 220, 0)), ("梯子", (255, 120, 0)),
                   ("清除", (0, 200, 200)), ("保存", (200, 200, 0)),
                   ("手动", (200, 0, 200)), ("刷新", (0, 180, 255))]
        for i, (text, color) in enumerate(buttons):
            x1 = i * btn_w
            x2 = (i + 1) * btn_w
            cv2.rectangle(btn_bar, (x1 + 1, 2), (x2 - 1, BTN_H - 2), (70, 70, 70), -1)
            cv2.putText(btn_bar, text, (x1 + 12, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
        # 高亮录制中按钮
        if self.recording_platform:
            cv2.rectangle(btn_bar, (1, 1), (btn_w - 1, BTN_H - 1), (0, 0, 255), 2)
        elif self.recording_ladder:
            cv2.rectangle(btn_bar, (btn_w + 1, 1), (2 * btn_w - 1, BTN_H - 1), (0, 0, 255), 2)
        full = np.vstack([map_display, btn_bar])
        return full

    def manual_select_region(self):
        """手动拖拽框选小地图区域（临时窗口，不影响主窗口固定尺寸）"""
        self._update_window_rect()
        frame = self._capture_window()
        print("\n=== 手动框选小地图 ===")
        print("在弹窗中拖拽框选小地图区域，按 Enter 确认，按 C 取消")

        sel_win = "Select Minimap (Enter=ok C=cancel)"
    ...
    def run(self):
        ...
        while True:
            ...
            self.frame_count += 1
            # 每30帧自动重新检测小地图区域（换地图后自动适配，约0.5秒一次）
