"""
键鼠控制模块
使用 Win32 SendInput 进行底层键鼠模拟，游戏兼容性较好。
...
class InputController:
    ...
    def _get_vk(self, key):
        ...
        if key_lower not in VK_MAP:
            raise ValueError(f"不支持的按键: {key}，可用按键: {list(VK_MAP.keys())}")
    ...
    def mouse_click(self, x=None, y=None, button="left"):
        ...
        inp_down.union.mi = MouseInput(dx=0, dy=0, mouseData=0, dwFlags=flag_down, time=0, dwExtraInfo=None)
        ctypes.windll.user32.SendInput(1, ctypes.byref(inp_down), ctypes.sizeof(inp_down))

        time.sleep(0.02)

        inp_up = Input(type=INPUT_MOUSE)
        inp_up.union.mi = MouseInput(dx=0, dy=0, mouseData=0, dwFlags=flag_up, time=0, dwExtraInfo=None)
