...
class MinimapRouteRecorder:
    def __init__(self):
        ...
        print("P=平台 L=梯子 C=清除 S=保存 R=重检 Q=退出\n")
    ...
    def draw(self, map_area, player_pos):
        ...
        y = 18
        status = "平台录制中(P停)" if self.recording_platform else ("梯子录制中(L停)" if self.recording_ladder else "待机")
    ...
    def run(self):
        win = "Minimap Route Recorder | P平台 L梯子 C清除 S保存 R重检 Q退出"
