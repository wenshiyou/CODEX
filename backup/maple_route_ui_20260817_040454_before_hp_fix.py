...
class MinimapRouteRecorder:
    ...
    def _detect_hp_mp_bars(self, frame):
        ...
        roi = frame[y_start:, :]
        # BGR阈值：HP=纯红，MP=青蓝
        hp_mask = (roi[:, :, 2] > 120) & (roi[:, :, 1] < 70) & (roi[:, :, 0] < 70)
        mp_mask = (roi[:, :, 0] > 120) & (roi[:, :, 2] < 100) & (roi[:, :, 1] > 80) & (roi[:, :, 1] < 200)
    ...
    def _read_bar_percent(self, frame, bar, color_type):
    ...
    def _check_auto_potion(self):
        ...
        # HP检测 — 百分比阈值，默认30%，吃药后延时1~20毫秒随机
        hp_pct = self._read_bar_percent(frame, self._hp_bar, "hp")
        hp_thresh = min(int(self._field_values.get("hp_value", "30") or "30"), 100)
        _debug_log("HP检测: pct=%s thresh=%d key=%s bar=%s" % (hp_pct, hp_thresh, cfg.get("hp_key"), self._hp_bar))
        if hp_pct is not None and cfg.get("hp_key") and hp_pct < hp_thresh:
            if now - self._last_hp_pot > self._hp_pot_delay:
                ...
                print("[自动吃药] HP=%.0f%% < %d%%, 按 %s" % (hp_pct, hp_thresh, cfg["hp_key"]))

        # MP检测 — 百分比阈值，默认30%，吃药后延时1~20毫秒随机
        mp_pct = self._read_bar_percent(frame, self._mp_bar, "mp")
        mp_thresh = min(int(self._field_values.get("mp_value", "30") or "30"), 100)
        _debug_log("MP检测: pct=%s thresh=%d key=%s bar=%s" % (mp_pct, mp_thresh, cfg.get("mp_key"), self._mp_bar))
        if mp_pct is not None and cfg.get("mp_key") and mp_pct < mp_thresh:
            if now - self._last_mp_pot > self._mp_pot_delay:
                ...
                print("[自动吃药] MP=%.0f%% < %d%%, 按 %s" % (mp_pct, mp_thresh, cfg["mp_key"]))
