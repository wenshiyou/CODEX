...
def select_region_on_screen():
    """全屏半透明覆盖窗口，拖拽选择屏幕区域，返回 (x1,y1,x2,y2) 屏幕坐标，取消返回 None"""
    result = [None]
    start = [None, None]
    rect_item = [None]
    ...
    def on_press(event):
        ...
        rect_item[0] = canvas.create_rectangle(event.x, event.y, event.x, event.y,
