...
import win32ui  # Windows UI DC，用于PrintWindow按窗口句柄截图（不截蒙板层）
...
BLUE_BOX_FILE = os.path.join(DATA_DIR, "blue_box_config.json")  # 蓝色框(一屏范围在小地图上的
# === 镜头死区检测：三个自动选的边缘区域（左下/左上/右中），三个都动+光点在动=镜头跟随，否则死区 ===
BG_DETECT_REGIONS_FILE = os.path.join(DATA_DIR, "bg_detect_regions.json")
BG_DETECT_DEFAULT_REGIONS = [
    {"x": 35, "y": 744, "w": 40, "h": 39},    # 左下（整个窗口坐标，含标题栏+边框）
...
class MinimapRouteRecorder:
    def __init__(self):
        ...
        try:
            ...
            if self.hwnd:
                ...
                if not self._load_region():
                    self._detect_minimap()  # 无保存区域时自动检测
        ...
        self._log_max = 500
        # 窗口大小固定：绑定时记录目标大小，运行中监控拉回（整个窗口尺寸，全局唯一一套坐标，截图/蒙板/锁定都用它）
        ...
        # YOLO怪物检测
        self._yolo_net = None
        self._yolo_unavailable = False  # 模型缺失/加载失败置True后直接短路,不再每帧查找/提示/检测;选模型或切方案时复位
        ...
        self._char_box_offset_x = 39  # 绿色人物框X偏移（F3面板调节）
        self._char_box_offset_y = 78  # 绿色人物框Y偏移（F3面板调节）
        self._char_offset_window = None  # F3偏移面板窗口引用
        ...
        # 偏移视觉反馈：数字变化后等1秒开始，持续5秒内每帧跑模板匹配，黄点实时跟随特征位置(加UI偏移)闪烁
        self._offset_feedback_start = 0  # 偏移修改时间戳
        self._offset_feedback_done = True  # 是否已设置持续期（避免重复设置）
        self._offset_feedback_until = 0  # 持续检测截止时间戳
    ...
    def _save_target_window_size(self):
        ...
        if self.hwnd and self.window_rect:
            ...
            _debug_log("[窗口固定] 目标大小已记录: %dx%d hwnd=%s rect=%s" % (
                self._target_window_size[0], self._target_window_size[1], self.hwnd, self.window_rect))
        else:
            _debug_log("[窗口固定] 目标大小记录失败: hwnd=%s window_rect=%s" % (self.hwnd, self.window_rect))
    ...
    def _load_region(self):
        ...
        if not os.path.exists(REGION_FILE):
            _debug_log("[区域加载] 配置文件不存在: %s" % REGION_FILE)
            return False
        try:
            ...
            if "map" in data and "minimap" in data:
                ...
                _debug_log("[区域加载] 成功: map=%dx%d minimap=%dx%d" % (
                    self.map_area_rect["width"], self.map_area_rect["height"],
                    self.minimap_rect["width"], self.minimap_rect["height"]))
                return True
            else:
                _debug_log("[区域加载] 失败: 配置文件缺少map或minimap字段")
        except Exception as e:
            _debug_log("[区域加载] 异常: %s" % e)
        return False
    ...
    def _detect_minimap(self, debug=True):
        ...
        if not debug:
            ...
            if old is not None:
                ...
                print("[自动刷新] 小地图区域首次检测: %dx%d" % (new_map["width"], new_map["height"]))

        self.minimap_rect = new_minimap
        self.map_area_rect = new_map
        _debug_log("[区域检测] _detect_minimap设置区域: map=%dx%d (left=%d,top=%d)" % (
            new_map["width"], new_map["height"], new_map["left"], new_map["top"]))
        self._save_region()
        self.last_player_pos = None
        ...
        if os.path.exists(calib_file):
            try:
                ...
                if yolo_path:
                    ...
                    self._yolo_net = None
                    self._yolo_unavailable = False  # 切换方案提供新路径，复位不可用标志允许重新加载
    ...
    def _open_char_offset_window(self):
        """F3绿色人物框偏移调节面板：X/Y偏移，输入或微调按钮，改完立即生效"""
        import tkinter as tk
        if not self._ensure_tk_root():
            return
        # 已打开则提到前台，不重复开
        if self._char_offset_window is not None:
            try:
                self._char_offset_window.lift()
                self._char_offset_window.focus_force()
                return
            except Exception:
                self._char_offset_window = None
        try:
            win = tk.Toplevel(self._tk_root)
        except Exception as e:
            _debug_log("[偏移面板] Toplevel创建失败: %s" % e)
            return
        self._char_offset_window = win
        win.title("绿色框偏移调节 (F3)")
        ...
        # 当前偏移值（用tk变量，初始化为实例当前值）
        var_x = tk.StringVar(value=str(int(getattr(self, '_char_box_offset_x', 39))))
        var_y = tk.StringVar(value=str(int(getattr(self, '_char_box_offset_y', 78))))

        def apply_values():
            """读取输入框并应用到实例（限界：-500~500，非法输入回退）"""
            try:
                vx = int(float(var_x.get()))
            except (ValueError, TypeError):
                vx = int(getattr(self, '_char_box_offset_x', 39))
                var_x.set(str(vx))
            try:
                vy = int(float(var_y.get()))
            except (ValueError, TypeError):
                vy = int(getattr(self, '_char_box_offset_y', 78))
                var_y.set(str(vy))
            vx = max(-500, min(500, vx))
            vy = max(-500, min(500, vy))
            self._char_box_offset_x = vx
            self._char_box_offset_y = vy
        ...
        def nudge(var, delta):
            ...
            try:
                ...
                cur = 0
            cur = max(-500, min(500, cur + delta))
            var.set(str(cur))
            apply_values()

        # 输入框内容变化时自动应用（实时生效）
        var_x.trace_add("write", lambda *a: apply_values())
        var_y.trace_add("write", lambda *a: apply_values())

        tk.Label(win, text="绿色人物框偏移调节", font=("微软雅黑", 11, "bold"),
                 bg="#404040", fg="white").grid(row=0, column=0, columnspan=5, pady=(10, 8))

        # X偏移行
        tk.Label(win, text="X偏移(左右):", font=("微软雅黑", 10), bg="#404040", fg="white",
                 width=10, anchor="e").grid(row=1, column=0, padx=(12, 4), pady=6)
        tk.Button(win, text="-10", width=4, command=lambda: nudge(var_x, -10),
                  bg="#606060", fg="white", relief="flat").grid(row=1, column=1, padx=2)
        tk.Button(win, text="-1", width=4, command=lambda: nudge(var_x, -1),
                  bg="#606060", fg="white", relief="flat").grid(row=1, column=2, padx=2)
        ent_x = tk.Entry(win, textvariable=var_x, width=6, font=("Consolas", 11), justify="center")
        ent_x.grid(row=1, column=3, padx=4)
        tk.Button(win, text="+1", width=4, command=lambda: nudge(var_x, 1),
                  bg="#2E7D32", fg="white", relief="flat").grid(row=1, column=4, padx=2)
        tk.Button(win, text="+10", width=4, command=lambda: nudge(var_x, 10),
                  bg="#2E7D32", fg="white", relief="flat").grid(row=1, column=5, padx=(2, 12))

        # Y偏移行
        tk.Label(win, text="Y偏移(上下):", font=("微软雅黑", 10), bg="#404040", fg="white",
                 width=10, anchor="e").grid(row=2, column=0, padx=(12, 4), pady=6)
        tk.Button(win, text="-10", width=4, command=lambda: nudge(var_y, -10),
                  bg="#606060", fg="white", relief="flat").grid(row=2, column=1, padx=2)
        tk.Button(win, text="-1", width=4, command=lambda: nudge(var_y, -1),
                  bg="#606060", fg="white", relief="flat").grid(row=2, column=2, padx=2)
        ent_y = tk.Entry(win, textvariable=var_y, width=6, font=("Consolas", 11), justify="center")
        ent_y.grid(row=2, column=3, padx=4)
        tk.Button(win, text="+1", width=4, command=lambda: nudge(var_y, 1),
                  bg="#2E7D32", fg="white", relief="flat").grid(row=2, column=4, padx=2)
        tk.Button(win, text="+10", width=4, command=lambda: nudge(var_y, 10),
                  bg="#2E7D32", fg="white", relief="flat").grid(row=2, column=5, padx=(2, 12))

        # 提示 + 关闭按钮
        tk.Label(win, text="正值向右/向下，负值向左/向上；改动立即生效",
                 font=("微软雅黑", 8), bg="#404040", fg="#BBBBBB").grid(
            row=3, column=0, columnspan=6, pady=(8, 2))
        tk.Button(win, text="关闭", width=10, command=on_close,
                  bg="#757575", fg="white", relief="flat").grid(row=4, column=0, columnspan=6, pady=(4, 12))

        # 定位到脚本窗口旁
        self._position_window(win, 360, 200)
        _debug_log("[偏移面板] 已打开 当前X=%s Y=%s" % (var_x.get(), var_y.get()))
    ...
    def _capture_window_native(self):
        """用PrintWindow API按游戏窗口句柄截图，不截覆盖在上面的蒙板层（纯游戏画面）。
        仅用于人物特征模板匹配回退路径，避免蒙板绿色框/黄点干扰匹配。别的功能继续用_capture_window。"""
        if self.hwnd is None or not self.window_rect:
            return None
        r = self.window_rect
        w = r.get("width", 0)
        h = r.get("height", 0)
        if w <= 0 or h <= 0:
            return None
        try:
            hdc_win = win32gui.GetDC(self.hwnd)
            if not hdc_win:
                return None
            ...
            # PrintWindow第3参数0=PW_CLIENTONLY只截客户区，和_capture_window的window_rect一致
            win32gui.PrintWindow(self.hwnd, hdc_mem.GetSafeHdc(), 0)
            bmp_bits = bmp.GetBitmapBits(True)
            img = np.frombuffer(bmp_bits, dtype=np.uint8).reshape(h, w, 4)
            img = img[:, :, :3].copy()  # BGRA->BGR，copy确保连续内存
            # 清理GDI资源
            win32gui.DeleteObject(bmp.GetHandle())
            hdc_mem.DeleteDC()
            win32gui.ReleaseDC(self.hwnd, hdc_win)
            return img
        except Exception as e:
            print("[原生截图] PrintWindow异常:", e)
            return None

    def _capture_game_clean(self):
        """截取纯游戏原画面（无蒙板干扰）：隐藏蒙板→mss截图→恢复蒙板。
        用于所有模板匹配/背景差异检测，确保识别的是游戏原图层，不是蒙板层。
        PrintWindow截不到DirectX画面，所以必须用mss+隐藏蒙板。"""
        _hidden = False
        if getattr(self, '_overlay_hwnd', None):
            user32.ShowWindow(self._overlay_hwnd, 0)  # SW_HIDE
            _hidden = True
            time.sleep(0.02)  # 等蒙板完全隐藏
        try:
            frame = self._capture_window()
            return frame
        finally:
            if _hidden and getattr(self, '_overlay_hwnd', None):
    ...
    def _detect_camera_motion(self, frame=None):
        ...
        if self._camera_state == "deadzone":
            ...
            if not dot_moving or motion_count < BG_MOTION_MIN_REGIONS:
                self._camera_still_count += 1
                if self._camera_still_count >= BG_STILL_FRAMES_TO_DEADZONE:
                    self._camera_state = "deadzone"
                    self._camera_still_count = 0
                    # 进入死区(镜头停止)：冻结此刻绿框位置，之后绿框钉住、光点在固定框内移动
    ...
    def _calib_fail(self, msg):
        ...
        if self._auto_calib_retry >= 3:  # 连续3次失败
            ...
            if self._overlay_hwnd:
                user32.ShowWindow(self._overlay_hwnd, 5)
    ...
    def _start_auto_calibration(self, axis='X'):
        ...
        if self._auto_calib_stage == 1:
            ...
            # 进入匹配阶段，隐藏蒙板避免光圈干扰模板匹配（整个stage=2期间都隐藏）
            if self._overlay_hwnd:
    ...
    def _capture_calib_template(self, screen_pos, color_tag):
        ...
        返回：True成功，False失败"""
        try:
            frame = self._capture_game_clean()  # 隐藏蒙板→截图→恢复，确保模板截到纯游戏原画面（无光圈干扰）
            if frame is None:
                return False
            h, w = frame.shape[:2]
            sx, sy = screen_pos
    ...
    def _match_calib_templates(self):
        ...
        if self._calib_green_template is None and self._calib_blue_template is None:
            return
        try:
            frame = self._capture_window()  # stage=2时蒙板已隐藏，这里直接截图（不能用_capture_game_clean，会恢复蒙板显示）
            if frame is None:
                return
            # 匹配绿点模板
    ...
    def _on_mouse(self, event, x, y, flags, param):
        """鼠标点击回调：标签页切换 + 路线页按钮"""
        # === 镜头死区检测：右键单击检测点框进入/退出移动模式 ===
        if event == cv2.EVENT_RBUTTONDOWN:
            _bg_regs = getattr(self, '_bg_regions', None)
            _bg_drag = getattr(self, '_bg_dragging', -1)
            if _bg_regs:
                if _bg_drag >= 0:
                    # 正在移动某个点，再次右键=保存并退出
                    print("[镜头检测] 保存检测点%d位置: x=%d y=%d" % (_bg_drag + 1, _bg_regs[_bg_drag]["x"], _bg_regs[_bg_drag]["y"]))
                    self._save_bg_regions()
                    self._bg_dragging = -1
                    return
                else:
                    # 没在移动，检查右键是否点在某个检测点框内
                    for _ri, _rr in enumerate(_bg_regs):
                        if _rr["x"] <= x <= _rr["x"] + _rr["w"] and _rr["y"] <= y <= _rr["y"] + _rr["h"]:
                            self._bg_dragging = _ri
                            print("[镜头检测] 进入检测点%d移动模式，右键保存" % (_ri + 1))
                            return
        # 移动模式下，鼠标移动更新该点位置
        if event == cv2.EVENT_MOUSEMOVE and getattr(self, '_bg_dragging', -1) >= 0:
            _di = self._bg_dragging
            self._bg_regions[_di]["x"] = max(0, x - self._bg_regions[_di]["w"] // 2)
            self._bg_regions[_di]["y"] = max(0, y - self._bg_regions[_di]["h"] // 2)
            return
        # 蓝色框校准模式：点击小地图记录角点
    ...
    def _load_char_templates(self):
        ...
        try:
            ...
            files.sort(key=lambda x: x[0])  # 按ID升序（旧的在前）
    ...
    def _show_offset_feedback(self):
        """偏移视觉反馈：数字变化后等1秒开始，持续5秒内每帧跑模板匹配，黄点实时跟随特征位置(加UI偏移)闪烁。
        数字再次变化时重置_start，重新等1秒开始新一轮。"""
        now_ms = time.time() * 1000

        # 阶段1：等1秒（等用户输完数字），到点后设置5秒持续期
        if not self._offset_feedback_done and self._offset_feedback_start > 0:
            elapsed = now_ms - self._offset_feedback_start
            if elapsed < 1000:
                return  # 还在等1秒
            self._offset_feedback_done = True
            self._offset_feedback_until = now_ms + 5000
            print("[偏移反馈] 开始持续检测5秒，黄点每帧跟随特征移动")

        # 阶段2：持续期内，每帧跑模板匹配，实时更新黄点位置
        if self._offset_feedback_until and now_ms < self._offset_feedback_until:
            blink_pos = None
            try:
                frame = self._capture_window()
                if frame is not None:
                    ...
                    if match:
                        ...
                        blink_pos = (mx + x_off, my + y_off)
            except Exception as e:
                print("[偏移反馈] 模板匹配异常:", e)
            if self._monster_overlay_data is None:
                self._monster_overlay_data = {}
            self._monster_overlay_data['blink_until'] = self._offset_feedback_until
            self._monster_overlay_data['blink_pos'] = blink_pos
        else:
            # 持续期结束，清除黄点
            if self._monster_overlay_data is not None:
                self._monster_overlay_data.pop('blink_pos', None)
                self._monster_overlay_data.pop('blink_until', None)
    ...
    def _monster_overlay_loop(self):
        ...
        使用Win32原生API（打包可靠），统一显示：角色橙色框 + 怪物绿框/连线 + 血条红点 + 蓝条蓝点"""
        try:
    ...
    def _win32_overlay_loop(self):
            ...
            try:
                ...
                # === 自动校准蒙板拖动（仅stage=1时，拖动绿点蓝点定特色位置）===
                    ...
                    try:
                        ...
                        # === 镜头死区检测：三个检测点框显示（白色框+序号，移动模式下红色闪烁）===
                        _bg_regs = getattr(self, '_bg_regions', None)
                        _bg_drag = getattr(self, '_bg_dragging', -1)
                        _cam_state = getattr(self, '_camera_state', 'deadzone')  # 状态机稳定结论(非瞬时值,人物不动会被锁死在死区)
                        if _bg_regs:
                            for _ri, _rr in enumerate(_bg_regs):
                                _rx, _ry, _rw, _rh = _rr["x"], _rr["y"], _rr["w"], _rr["h"]
                                # 颜色：右键拖动中的框=白色高亮；否则following(镜头移动)=绿，deadzone(镜头没动)=红
                                if _bg_drag == _ri:
                                    _rc = 0xFFFFFF
                                elif _cam_state == "following":
                                    _rc = 0x00FF00  # 绿：镜头正在移动
                                else:
                                    _rc = 0x0000FF  # 红：镜头没动(死区)
                                _pen_bg = gdi32.CreatePen(0, 1, _rc)
                                if _pen_bg:
                                    gdi_objs.append(_pen_bg)
                                _old_pen_bg = gdi32.SelectObject(hdc, _pen_bg)
                                gdi32.SelectObject(hdc, gdi32.GetStockObject(5))  # 空刷
                                gdi32.Rectangle(hdc, _rx, _ry, _rx + _rw, _ry + _rh)
                                gdi32.SelectObject(hdc, _old_pen_bg)
                                # 序号标注
                                gdi32.SetTextColor(hdc, _rc)
                                gdi32.SetBkMode(hdc, 1)
                                _label = str(_ri + 1)
                                gdi32.TextOutW(hdc, _rx + 2, _ry + 1, _label, len(_label))
                        ...
                        if data:
                            ...
                            if char_pos:
                                if first_draw[0]:
                                    first_draw[0] = False
                                    _debug_log("[蒙板绘制] 首次绘制人物绿色框 at %s" % (char_pos,))
                                cx, cy = char_pos
                                # 人物绿色框：宽126高136，光点锁定坐标+偏移（F3面板可调）
                                cx = cx + getattr(self, '_char_box_offset_x', 39)
                                cy = cy + getattr(self, '_char_box_offset_y', 78)
                                char_box_w = 126
                                char_box_h = 136
                                box_left = cx - char_box_w // 2
                                box_top = cy - char_box_h // 2
                                box_right = box_left + char_box_w
                                box_bottom = box_top + char_box_h
                                pen = gdi32.CreatePen(0, 2, 0x00FF00)
                                if pen:
                                    gdi_objs.append(pen)
                                ...
                                # 偏移反馈黄点：用户改UI偏移后等3秒触发，在人物位置(已加char_x_offset/y)闪烁5秒
                                blink_until = data.get('blink_until', 0)
                                blink_pos = data.get('blink_pos', None)
                                now_ms_blink = time.time() * 1000
                                if blink_until and now_ms_blink < blink_until and blink_pos:
                                    # 闪烁：每250ms切换显示/隐藏
                                    if int((blink_until - now_ms_blink) / 250) % 2 == 0:
                                        bx0, by0 = blink_pos  # 用人物特征模板匹配位置（已加UI偏移），不用光点锁定
                                        yellow_brush = gdi32.CreateSolidBrush(0x00FFFF)  # 黄色BGR
                                        if yellow_brush:
                                            gdi_objs.append(yellow_brush)
                                        old_yellow = gdi32.SelectObject(hdc, yellow_brush)
                                        gdi32.Ellipse(hdc, bx0 - 5, by0 - 5, bx0 + 6, by0 + 6)  # 半径5直径11，原17的三分之二
                                        gdi32.SelectObject(hdc, old_yellow)
                                # 第一步：显示比例值（验证比例用）
                                _ratio = data.get('dot_ratio')
                                if _ratio:
                                    _rx, _ry = _ratio
                                    _ratio_txt = "rx=%.3f ry=%.3f" % (_rx, _ry)
                                    gdi32.SetTextColor(hdc, 0x00FF00)  # 绿色
                                    gdi32.SetBkMode(hdc, 1)  # 透明背景
                                    gdi32.TextOutW(hdc, box_right + 5, box_top, _ratio_txt, len(_ratio_txt))
        ...
        hwnd = user32.CreateWindowExW(
            WS_EX_LAYERED | WS_EX_TOPMOST | WS_EX_TOOLWINDOW,  # TOOLWINDOW让蒙板不进任务栏(避免任务栏Overlay按钮反复闪烁),不影响透明置顶
        ...
        msg = MSG()
        _last_overlay_geom = None  # 上次定位几何，只在变化时重新SetWindowPos，避免反复置顶导致任务栏闪烁
        while self._monster_overlay_running:
            try:
                if self.hwnd and self.window_rect:
                    ...
                    if _cur_geom != _last_overlay_geom:  # 仅游戏窗口移动/缩放时重新定位置顶，位置不变不重复抢Z序(防任务栏抖动)
                        user32.SetWindowPos(hwnd, -1, wr['left'], wr['top'],
                                            wr['width'], wr['height'], 0x0050)
                        _last_overlay_geom = _cur_geom
                elif first_draw[0]:
    ...
    def _select_yolo_model(self):
        ...
        if path:
            ...
            self._yolo_unavailable = False  # 用户手动选了新模型，复位不可用标志允许重试
    ...
    def _init_yolo(self):
        ...
        模型缺失/加载失败置_yolo_unavailable并只提示一次，之后直接短路返回False，
        不再每帧查文件/尝试加载/检测；仅当用户重新选模型或切换方案(复位_yolo_unavailable)才重试。"""
        if self._yolo_net is not None:
            return True
        if getattr(self, '_yolo_unavailable', False):
            return False  # 已确认不可用，直接短路，不重复查找/提示/检测
        # 优先使用手动选择的模型路径
        ...
        if not os.path.exists(model_path):
            print("[YOLO] 未找到模型文件，请点击'怪物数据'选择.onnx模型（选好前不再重复提示与检测）")
        ...
        try:
            ...
            print("[YOLO] 加载失败(不再自动重试，请重新选择模型):", e)
            self._yolo_unavailable = True
            return False
    ...
    def _detect_monsters(self, frame):
        ...
        detections = []
        for row in out:  # out=forward()[0]即(300,6),每行6值[x1,y1,x2,y2,score,cls]
    ...
    def lock_screen_from_dot(self):
        ...
        if not r or r.get("width", 0) <= 0 or r.get("height", 0) <= 0:
            if getattr(self, 'frame_count', 0) % 60 == 0:
                _debug_log("[光点锁定] 失败: map_area_rect无效=%s" % r)
            return None
        if not self._player_map_pos:
            if getattr(self, 'frame_count', 0) % 60 == 0:
                _debug_log("[光点锁定] 失败: _player_map_pos=None(光点未检测到)")
            return None
        mx, my = self._player_map_pos  # 修复: 解包元组, 不是重复赋值
        # 用整个窗口尺寸计算坐标（与_capture_window截图、蒙板绘制同一套坐标系；禁止另用客户区尺寸造成两套坐标偏移）
        ...
        if self._blue_box:
            ...
            if self._camera_state == "deadzone" and frozen_pos is not None:
                ...
                box_x, box_y = follow_pos  # 镜头移动：绿框以光点为中心
            ...
            # 未校准：用缩进后的有效区域归一化（去掉小地图黑边，左4右5上3下3）
            mw = r["width"]
            mh = r["height"]
            left_m, right_m, top_m, bottom_m = 4, 5, 3, 3
            eff_w = mw - left_m - right_m
            eff_h = mh - top_m - bottom_m
            if eff_w > 0 and eff_h > 0:
                rx = (mx - left_m) / float(eff_w)
                ry = (my - top_m) / float(eff_h)
            else:
                rx = mx / float(mw)
                ry = my / float(mh)
            rx = max(0.0, min(1.0, rx))
        ...
        sx = int(win_w * rx)
        sy = int(win_h * ry)
        self._last_dot_ratio = (rx, ry)  # 保存比例值，供蒙板显示验证
        if getattr(self, 'frame_count', 0) % 20 == 0:
            _debug_log("[光点锁定] 光点(%d,%d) %s归一化(%.3f,%.3f) 屏幕(%d,%d)" % (mx, my, mode, rx, ry, sx, sy))
        return (sx, sy)
    ...
    def _get_player_screen_pos(self, frame):
        """获取人物在游戏画面中的坐标: 优先光点归一化(lock_screen_from_dot, 不用倍率, 稳准),
        光点归一化不可用(无光点/无地图框)才回退模板匹配; 再失败宽限期用上次位置.
        人物特征偏移(char_x_offset/char_y_offset)作用于所有返回路径, 确保UI偏移调节生效."""
        # 人物特征偏移（UI上X偏移/Y偏移输入框，作用于所有返回的人物坐标）
        x_off = int(self._field_values.get("char_x_offset", "0") or "0")
        y_off = int(self._field_values.get("char_y_offset", "0") or "0")
        # 1) 优先: 光点归一化锁定(方案核心, 不用倍率)
        pos = self.lock_screen_from_dot()
        if pos is not None:
            self._last_char_match_pos = pos  # 保存未加偏移的真实位置(供ROI回退用)
            self._last_char_match_time = time.time() * 1000
            if getattr(self, 'frame_count', 0) % 60 == 0:
                _debug_log("[人物坐标] 光点锁定成功: %s 偏移(%+d,%+d)" % (pos, x_off, y_off))
            return (pos[0] + x_off, pos[1] + y_off)
        # 2) 回退: 模板匹配(兜底, 光点不可用时)
        # 用PrintWindow原生截图（不截蒙板层），避免蒙板绿色框/黄点干扰人物特征匹配
        native_frame = self._capture_game_clean()
        match_frame = native_frame if native_frame is not None else frame
        match = self._match_character(match_frame)
        if match:
            mx, my, _ = match
            result = (mx + x_off, my + y_off)
            if getattr(self, 'frame_count', 0) % 60 == 0:
                _debug_log("[人物坐标] 模板匹配回退: %s" % (result,))
            return result
        # 3) 稳远期: 宽限用上次成功位置
        last_pos = getattr(self, '_last_char_match_pos', None)
        ...
        if last_pos and now_ms - last_time < 1500:
            if getattr(self, 'frame_count', 0) % 60 == 0:
                _debug_log("[人物坐标] 宽限期用上次位置: %s 偏移(%+d,%+d)" % (last_pos, x_off, y_off))
            return (last_pos[0] + x_off, last_pos[1] + y_off)
        # 4) 全部失败: 返回None（第一步验证比例，不用兜底坐标混淆）
        if getattr(self, 'frame_count', 0) % 60 == 0:
            _debug_log("[人物坐标] 全部失败: 返回None(光点无+模板无+宽限超时)")
        return None
    ...
    def run(self):
        ...
        # 防灰屏：窗口创建后立即渲染一帧静态背景并泵一次Windows消息，让窗口先响应。
        # 否则冷启动(解包+Defender扫描DLL)时首帧检测到第一次waitKey前若超过约5秒，窗口会被判未响应灰屏。
        ...
        while True:
            ...
            if self.hwnd:  # 人物屏幕位置每帧检测（绿框跟随人物实时刷新）
                try:
                    ...
                    print("[主循环] 帧检测异常:", _e)  # 落实AGENTS第8条,不再静默吞异常
            ...
            self._check_hotkeys()
            # 蓝色框校准模式：方向键微调选中角点，S保存，ESC退出
            if self._calibrating_blue_box:
                for _vk in [0x25, 0x26, 0x27, 0x28, 0x53, 0x51]:
                    if user32.GetAsyncKeyState(_vk) & 0x8000:
                        if _vk not in getattr(self, '_bluebox_key_state', {}):
                            self._bluebox_key_state = {}
                        if not self._bluebox_key_state.get(_vk, False):
                            self._handle_blue_box_key(_vk)
                        self._bluebox_key_state[_vk] = True
                    else:
                        if not hasattr(self, '_bluebox_key_state'):
                            self._bluebox_key_state = {}
                        self._bluebox_key_state[_vk] = False
            ...
            try:
                if hasattr(self, '_tk_root') and self._tk_root is not None:
                    has_win = (getattr(self, '_save_window', None) is not None or
                               ...
                               getattr(self, '_char_offset_window', None) is not None)
                    if has_win:
            ...
            # 同步人物位置到蒙板（不依赖_running，确保测试时也能显示人物框）
            if self._monster_overlay_running:
                if self._monster_overlay_data is None:
                    self._monster_overlay_data = {}
                if self._player_screen_pos:
                    ...
                    # 同步比例值（第一步验证比例用）
                    _ratio = getattr(self, '_last_dot_ratio', None)
                    if _ratio:
                        self._monster_overlay_data["dot_ratio"] = _ratio
                    if getattr(self, 'frame_count', 0) % 60 == 0:
                        _debug_log("[蒙板同步] char_pos=%s ratio=%s" % (self._player_screen_pos, _ratio))
                else:
                    if getattr(self, 'frame_count', 0) % 60 == 0:
                        _debug_log("[蒙板同步] char_pos未同步: _player_screen_pos=None")
            if self._running:
                try:
                    if self._monster_overlay_data is None:
                        self._monster_overlay_data = {}
...
if __name__ == "__main__":
    import faulthandler; faulthandler.enable()  # native崩溃(access violation)时打印Python调用栈到stderr,辅助定位闪退
    # === 管理员权限检查 ===
