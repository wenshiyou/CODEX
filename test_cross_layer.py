...
def test_cross_candidates_returned():
    """X走进技能射程、Y仍超带且非同平台→cross并返回cross_candidates(供_try_platform_transition做屏幕→小地图转换);
    用户2026-09-11新X规则:X还远时哪怕Y超也先pursue走近、cross_candidates为空。"""
    # 怪中心400(X差100≤射程150=已走近),脚140(Y差160>60,在上方另一层),非同平台→cross
    monsters = [
        (380, 100, 420, 140, 0.9),
    ]
    ...
    check("cross_candidates格式[(dist,cx,cy)]", d['cross_candidates'][0][1] == 400)
    # 对照:同一只上层怪但X还远(中心920,X差620>射程)→先pursue水平走近,不提前cross、无cross候选
    d2 = cl.select_combat_target(
        px=300, py=300, monsters=[(900, 100, 940, 140, 0.9)], selected_platforms=[],
        skill_range=150, far_range=500,
        target_cx=None, target_cy=None, target_alive=False,
        is_on_platform=lambda cx, cy: False, get_monster_platform=lambda cx, cy: None,
        probe_side=1, probe_switched=False, cur_cross=None,
        attack_y_up=60, attack_y_down=30, allow_cross=True,
    )
    check("X远Y超=先pursue走近(不原地cross)", d2['state'] == 'pursue')
    check("X远时cross_candidates为空", len(d2.get('cross_candidates', [])) == 0)


def test_cross_disabled_no_candidates():
    """allow_cross=False时:X已进射程但Y超带的怪不进cross(也非同层cand)→idle且cross_candidates为空"""
    monsters = [(380, 100, 420, 140, 0.9)]  # 中心400(X差100≤射程),脚140(Y差160>60,上层怪)
    d = cl.select_combat_target(
        px=300, py=300, monsters=monsters, selected_platforms=[],
        skill_range=150, far_range=500,
        target_cx=None, target_cy=None, target_alive=False,
