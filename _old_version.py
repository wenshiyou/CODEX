...
class MinimapRouteRecorder:
    ...
    def _detect_minimap(self, debug=True):
        ...
        LEFT_OFFSET = 1  # 左边增加1像素白边（原6→2→1，逐步向左扩大左侧显示范围）
    ...
    def draw(self, map_area, player_pos):
        ...
        # 【光点锁定可视化】在小地图上画一个方框，直观框住"黄色光点(人物)"，框住锁定范围
        if getattr(self, '_player_map_pos', None):
            _pmx, _pmy = self._player_map_pos[0], self._player_map_pos[1]
            # 原始小地图坐标 → map_display 缩放坐标
            _sc = scale_x  # map_display 的 X 缩放(render_w/w)
            _scy = scale_y
            _bx = int(_pmx * _sc)   # 光点在 map_display 上的X
            _by = int(_pmy * _scy)  # 光点在 map_display 上的Y
            # 方框大小: 相对固定(可调), 框住光点及周围
            _hw = int(28)   # 半宽
            _hh = int(24)   # 半高
            cv2.rectangle(map_display, (_bx - _hw, _by - _hh), (_bx + _hw, _by + _hh), (0, 255, 0), 2)  # 绿框框住光点
            _debug_log("[光点锁定框] 光点(%d,%d) 屏框(%d,%d) (%d,%d)" % (_pmx, _pmy, _bx, _by, _hw * 2, _hh * 2))
    ...
    def _save_blue_box(self):
        ...
        try:
            ...
            print("[蓝色框] 保存成功: %dx%d" % (self._blue_box["width"], self._blue_box["height"]))
        except Exception as e:
            print("[蓝色框] 保存失败:", e)
    ...
    def _start_blue_box_calibration(self):
        ...
        self._add_log("蓝色框校准：依次点击小地图四个角（左上→右上→右下→左下）")
        print("[蓝色框] 进入校准模式，请点击小地图四个角")

    def _handle_blue_box_click(self, map_x, map_y):
        """校准模式下处理小地图点击：记录角点，四个点齐了计算蓝色框大小"""
        if not self._calibrating_blue_box:
            return False
        # 点击已有角点附近=选中该角点用于微调
        for i, (cx, cy) in enumerate(self._blue_box_corners):
            if abs(map_x - cx) < 8 and abs(map_y - cy) < 8:
                self._selected_corner = i
                self._add_log("选中角点%d，可用方向键微调" % (i + 1))
                return True
        # 记录新角点
        if len(self._blue_box_corners) < 4:
            self._blue_box_corners.append((int(map_x), int(map_y)))
            self._selected_corner = len(self._blue_box_corners) - 1
            self._add_log("角点%d: (%d, %d)" % (len(self._blue_box_corners), map_x, map_y))
            if len(self._blue_box_corners) == 4:
    ...
    def _calc_blue_box_from_corners(self):
        """四个角点齐了，计算蓝色框宽高（取对角点距离的平均值）"""
        corners = self._blue_box_corners
        # 排序：左上、右上、右下、左下
        ...
        if width > 10 and height > 10:
            ...
            self._add_log("蓝色框大小: %dx%d，方向键微调后按S保存" % (width, height))
            print("[蓝色框] 计算大小: %dx%d" % (width, height))
        else:
            self._add_log("蓝色框太小，请重新点击四个角")

    def _handle_blue_box_key(self, key_code):
        """校准模式下键盘方向键微调选中的角点，S保存，ESC退出"""
        if not self._calibrating_blue_box:
            return False
        if self._selected_corner < 0 or self._selected_corner >= len(self._blue_box_corners):
            return False
        cx, cy = self._blue_box_corners[self._selected_corner]
        step = 1
        if key_code == 0x25:  # 左
            cx -= step
        ...
        # 边界保护
        r = self.map_area_rect
        cx = max(0, min(cx, r["width"] - 1))
        cy = max(0, min(cy, r["height"] - 1))
        self._blue_box_corners[self._selected_corner] = (cx, cy)
        if len(self._blue_box_corners) == 4:
            self._calc_blue_box_from_corners()
        return True

    def _draw_blue_box(self, map_frame):
        """在小地图帧上绘制蓝色框（校准模式画角点，正常模式画框）"""
        if self._calibrating_blue_box:
            ...
            if len(self._blue_box_corners) == 4:
                pts = np.array(self._blue_box_corners, np.int32).reshape((-1, 1, 2))
    ...
    def run(self):
        ...
        while True:
            ...
            if self._calibrating_blue_box:
                for _vk in [0x25, 0x26, 0x27, 0x28, 0x53, 0x1B]:
