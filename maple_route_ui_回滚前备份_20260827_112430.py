...
YELLOW_H_LOW = 26  # 黄色H下限（从20提高到26：人物光点核心H=27~30，排除偏橙/偏红的边缘(如18~24的红色星标)被误判成黄色）
...
class MinimapRouteRecorder:
    ...
    def _show_msg(self, kind, title, message):
        """定位在脚本窗口上方的消息框（替代messagebox，避免屏幕居中）"""
        import tkinter as tk
        from tkinter import messagebox
        tmp = tk.Toplevel(self._tk_root)
        tmp.withdraw()
        self._position_window(tmp, 300, 150)
        tmp.update()
        try:
            if kind == "warning":
                return self._show_msg("warning", title, message)
            elif kind == "info":
                return self._show_msg("info", title, message)
            else:
                return self._show_msg("yesno", title, message)
        finally:
            tmp.destroy()
            ...
            for plan in mp["plans"]:
                ...
                def make_activate(pid_):
                    def on_click(event):
                        ...
                        self._close_window("_plan_window")
                        self._open_plan_window()  # 刷新颜色
                    return on_click
    ...
    def _open_clear_window(self):
        ...
        def on_double(event):
            ...
            if itype == "map":
                ...
                if self._show_msg("yesno", "确认", "您确定要删除此方案吗？"):
    ...
    def find_player_dot(self, map_area):
        """【模块B】检测小地图上人物黄色光点（人物光点是实心纯黄圆形，HSV颜色过滤框定后取外接矩形中心=圆心）
        原理：HSV颜色过滤找黄色区域(人物光点纯黄极亮)，选中最像人物的候选(面积x亮度综合最大)，取该轮廓外接矩形中心作为圆心。
        返回：(x, y) 人物光点圆心坐标；找不到返回None"""
        hsv = cv2.cvtColor(map_area, cv2.COLOR_BGR2HSV)  # 转换到HSV颜色空间
        lower = np.array([YELLOW_H_LOW, YELLOW_S_LOW, YELLOW_V_LOW])  # 黄色下限(收紧后, 纯黄)
        upper = np.array([YELLOW_H_HIGH, 255, 255])  # 黄色上限
        h, w = map_area.shape[:2]  # 获取小地图尺寸
        mask = cv2.inRange(hsv, lower, upper)  # 黄色过滤mask
        # mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((2, 2), np.uint8))  # 注释掉MORPH_OPEN，避免吃掉小光点
        cnts, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # 找所有黄色轮廓
        if cnts:
            # 对每个轮廓算面积/外接矩形/平均亮度，选最像人物的(面积x亮度综合最大)
            candidates = []
            v_channel = hsv[:, :, 2]  # V通道(亮度)
            for c in cnts:
                area = cv2.contourArea(c)  # 轮廓面积
                if area <= 0:
                    continue  # 跳过面积为0的无效轮廓
                x, y, bw, bh = cv2.boundingRect(c)  # 外接矩形
                cx = x + bw // 2  # 外接矩形中心X(=实心圆形圆心)
                cy = y + bh // 2  # 外接矩形中心Y(=实心圆形圆心)
                # 计算该轮廓区域的平均亮度(V值)
                ...
                candidates.append((cx, cy, area, mean_val))  # 保存候选：外接矩形中心+面积+平均亮度
            if candidates:
                if getattr(self, 'frame_count', 0) % 10 == 0:
                    _debug_log("[光点检测] 候选数=%d candidates=%s" % (len(candidates), [(c[0], c[1], c[2], c[3]) for c in candidates]))  # 调试日志
                # 取面积x亮度综合最大的轮廓(人物光点最大最亮)，用外接矩形中心(=圆心)作为光点正中心
                best = max(candidates, key=lambda p: p[2] * p[3])  # 面积×亮度 综合最大→人物光点
                best_cx, best_cy, best_area, best_mean = best
                self.last_player_pos = (best_cx, best_cy)  # 更新上次位置
                return (best_cx, best_cy)  # 返回外接矩形中心(圆心, 光点正中心)
    ...
    def _effective_scale(self):
        ...
        base_x = getattr(self, '_calibrated_scale_x', 0.0)
    ...
    def draw(self, map_area, player_pos):
        ...
        _fs = 0.5   # 字体约为原来的2/3
        _left = 2   # 左移约0.6~1px(整像素取2，少1px)
        _baseline = render_h - 3
