...
class MinimapRouteRecorder:
    def __init__(self):
        ...
        self._static_detection_tracker = {}  # 静态位置过滤 {pos_key: (x, y, first_seen_ms)}
    ...
    def _move_to(self, player_pos, target_x, target_y):
        ...
        # === 跳跃反馈检测已禁用：按用户要求，失败后先检测怪，没怪再重新跳，不做5秒禁跳 ===
        jump_blocked = False
        ...
        if self._climb_state == "climbing":
            ...
            if abs(cdy) <= 4:
                # 到达目标高度，多按1.5秒上方向键确保完全到上层平台
                if not getattr(self, '_climb_arrived_time', 0):
                    self._climb_arrived_time = now_ms
                    _debug_log("[爬梯-步骤5] 到达目标高度！当前Y=%.0f 目标Y=%.0f 差值=%.0f，继续按上1.5秒确保到上层" % (
                        py, self._climb_target_y, cdy))
                elif now_ms - self._climb_arrived_time > 1500:
                    _debug_log("[爬梯-步骤5] 已多按1.5秒上键，攀爬完成，重新检测怪物")
                    self._climb_arrived_time = 0
                    self._reset_climb()
                    return False
            else:
                self._climb_arrived_time = 0
    ...
    def _detect_hp_mp_bars(self, frame):
        ...
        h, w = frame.shape[:2]
        y_start = max(0, h - 25)
        self._hp_mp_search_y = y_start  # 保存检测区域y坐标，供蒙板显示
        roi = frame[y_start:, :]
        ...
        _hp_mask_sum = int(hp_mask.sum())
        _mp_mask_sum = int(mp_mask.sum())
        hp_bar = self._find_longest_hbar(hp_mask, y_start)
        _debug_log("[HP检测调试] 帧尺寸=%dx%d 检测区y=%d HP掩码像素=%d MP掩码像素=%d find_longest返回=%s" % (
            w, h, y_start, _hp_mask_sum, _mp_mask_sum, hp_bar))
        mp_bar = None
        if hp_bar:
            hx, hy, hw = hp_bar
        ...
        # 稳定性缓存：只防位置跳变，不限制宽和x范围（HP总宽可能400+）
        if not hasattr(self, '_hp_bar_stable'):
            self._hp_bar_stable = None
        if not hasattr(self, '_mp_bar_stable'):
            self._mp_bar_stable = None
        # HP：只要检测到就接受，和缓存x差<100则更新缓存
        if hp_bar:
            if self._hp_bar_stable is None or abs(hp_bar[0] - self._hp_bar_stable[0]) < 100:
                self._hp_bar_stable = hp_bar
        if self._hp_bar_stable:
            hp_bar = self._hp_bar_stable
        # MP：只要检测到就接受
        if mp_bar:
            if self._mp_bar_stable is None or abs(mp_bar[0] - self._mp_bar_stable[0]) < 100:
                self._mp_bar_stable = mp_bar
        if self._mp_bar_stable:
            mp_bar = self._mp_bar_stable
        _debug_log("血条检测: hp=%s mp=%s (HP掩码=%d MP掩码=%d)" % (
            hp_bar, mp_bar, _hp_mask_sum, _mp_mask_sum))
    ...
    def _detect_monster_hp_bars(self, frame, search_areas=None):
        ...
        if frame is None:
            return []
        h, w = frame.shape[:2]
        # 纯绿色检测：B<60, G>180, R<60（样本色(0,243,0)，容忍抗锯齿）
        b, g, r = frame[:, :, 0], frame[:, :, 1], frame[:, :, 2]
        green_mask = (b < 60) & (g > 180) & (r < 60)
    ...
    def _check_auto_potion(self):
        ...
        # 先用缓存的HP/MP条位置更新蒙板竖框显示（不受截图异常影响）
        if self._monster_overlay_running:
            if self._monster_overlay_data is None:
                self._monster_overlay_data = {}
            _debug_log("[加药-蒙板更新] _hp_bar=%s _mp_bar=%s hp_thresh=%d mp_thresh=%d" % (
                self._hp_bar, self._mp_bar, hp_thresh, mp_thresh))
            if hasattr(self, '_hp_mp_search_y'):
                self._monster_overlay_data['hp_mp_search_y'] = self._hp_mp_search_y
            if self._hp_bar:
        ...
        # 截图前先更新窗口矩形，防止被其他函数（如_detect_minimap）改成小窗口
        self._update_window_rect()
        frame = self._capture_window()
        if frame is None:
            return
        # 校验截图尺寸：截到小窗口(如160x28)时跳过加药检测，但蒙板已用缓存值更新
        fh, fw = frame.shape[:2]
        if fw < 800 or fh < 600:
            _debug_log("[加药] 截图尺寸异常(%dx%d) hwnd=%s，跳过加药检测" % (fw, fh, self.hwnd))
            return
    ...
    def _combat_tick(self):
        ...
        if now - self._last_yolo_check > 30:
            ...
            if frame is not None:
                ...
                # === 静态位置过滤：1秒不动→放弃检测并禁用该坐标3秒，3秒后解禁重新检测 ===
                _now_ms_static = int(time.time() * 1000)
                # 清理过期的禁用坐标
                if hasattr(self, '_static_banned_positions'):
                    self._static_banned_positions = {k: v for k, v in self._static_banned_positions.items() if v > _now_ms_static}
                else:
                    self._static_banned_positions = {}
                _filtered_static = []
                _new_tracker = {}
                for (x1, y1, x2, y2, score) in merged:
                    _key = "%d_%d" % (x1 // 15, y1 // 15)
                    # 坐标在禁用列表中（3秒内），直接跳过不检测
                    if _key in self._static_banned_positions:
                        continue
                    if _key in self._static_detection_tracker:
                        _old_x, _old_y, _first_seen = self._static_detection_tracker[_key]
                        if abs(x1 - _old_x) < 15 and abs(y1 - _old_y) < 15:
                            if _now_ms_static - _first_seen > 1000:
                                # 超过1秒不动，放弃检测，并禁用该坐标3秒
                                self._static_banned_positions[_key] = _now_ms_static + 3000
                                _debug_log("[静态过滤] 坐标(%d,%d)静止超1秒，放弃检测并禁用3秒" % (x1, y1))
                                continue
                            _new_tracker[_key] = (x1, y1, _first_seen)
                        else:
                            _new_tracker[_key] = (x1, y1, _now_ms_static)
                    else:
                        _new_tracker[_key] = (x1, y1, _now_ms_static)
                    _filtered_static.append((x1, y1, x2, y2, score))
                self._static_detection_tracker = _new_tracker
                if len(merged) != len(_filtered_static):
                    _debug_log("[静态过滤] 检测%d只，过滤掉%d只(含禁用坐标)，剩%d只，禁用坐标%d个" % (
                        len(merged), len(merged) - len(_filtered_static), len(_filtered_static), len(self._static_banned_positions)))
                merged = _filtered_static
        ...
        if near_same:
        ...
        elif same_platform_candidates and candidate_pool:
            # 同平台没有1000px以内的，看其他平台有没有更近的怪
            nearest_all_dist = candidate_pool[0][0]
            nearest_same_dist = same_platform_candidates[0][0]
            nearest_all_is_diff_platform = not any(
                abs(candidate_pool[0][1] - m[1]) < 30 and abs(candidate_pool[0][2] - m[2]) < 50
                for m in same_platform_candidates)
            if nearest_all_is_diff_platform and nearest_same_dist - nearest_all_dist > 150:
                _debug_log("[打怪] 同平台最近%dpx，其他平台最近%dpx，换平台" % (nearest_same_dist, nearest_all_dist))
                self._release_combat_move()
                self._combat_locked_target = None
                self._combat_had_target = False
                return
            ...
            # 同平台没有怪：选最近的任意平台怪
            if not same_platform_candidates and candidate_pool:
                _debug_log("[目标选择] 同平台无怪，选最近的任意平台怪(%dpx)" % candidate_pool[0][0])
                monster_dists = candidate_pool
            elif not same_platform_candidates:
        ...
        if self._combat_locked_target:
            ...
            if locked_found:
                ...
                if ld <= atk_dist:
                    ...
                    for (bx, by, bw, bh) in self._monster_hp_bars:
                        bcx = bx + bw // 2
                        if abs(bcx - lcx2) < 80 and by < lcy2 + 20:
                            has_blood = True
                            break
                    if has_blood:
        ...
        # --- 主攻：目标在距离内，无固定冷却，只用随机延时控制间隔 ---
        atk_key = fight_cfg.get("atk1_key", "")
        if not skill_cast and atk_key:
            atk_dist = fight_cfg.get("atk1_distance", 150)
            last = self._attack_last.get("atk1", 0)
            min_gap = max(20, random.randint(0, skill_rand))  # 随机延时，最小20ms
            if t_dist <= atk_dist and now - last > min_gap:
                # 攻击前诊断：检测血条情况
                _blood_atk = False
                for (bx, by, bw, bh) in self._monster_hp_bars:
                    bcx = bx + bw // 2
                    if abs(bcx - t_cx) < 80 and by < t_cy + 20:
                        _blood_atk = True
                        break
                _missed_count = len(getattr(self, '_combat_missed_positions', {}))
                if _blood_atk:
                    _debug_log("[攻击诊断] 当前攻击：检测到血条，目标(%d,%d)距离%dpx，拉黑位置%d个" % (t_cx, t_cy, t_dist, _missed_count))
                else:
                    _debug_log("[攻击诊断] 当前攻击：检测到怪物但无血条，目标(%d,%d)距离%dpx，血条数%d，拉黑位置%d个" % (
                        t_cx, t_cy, t_dist, len(self._monster_hp_bars), _missed_count))
