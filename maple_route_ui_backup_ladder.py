        ...
        if now - self._last_yolo_check > 30:
            ...
            if frame is not None:
                ...
                if len(merged) != len(_filtered_static):
                    _debug_log("[静态过滤] 检测%d只，过滤掉%d只静止超1秒的背景误检，剩%d只" % (
                        len(merged), len(merged) - len(_filtered_static), len(_filtered_static)))
